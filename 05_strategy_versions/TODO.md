...
| # | 事项 | 出处 |
|---|---|---|
| 50 | **14:45 的 B1 状态 vs RiskDecision 优先级顺序**：`review_core.classify` 里 B1 的 P0/P1/P2 短路 return 排在 `high_risk` 之前 ⇒ RiskDecision 判「高」（如已触发止损线）而 B1 按实时价只判 P2 时，**显示的优先级比风险应有的更低**（P2「待收盘确认」而非 P1「减仓/止损评估」）。<br>两边都有道理：README §决策优先级写「risk_control 拥有否决权」，但 B1 用的是 **14:45 实时价**、RiskDecision 可能来自**前一日 17:00**（`risk_source_date` 会回退最近一份），拿旧依据压过新读数也不对。<br>已先修「理由丢失」那半（现在会附「⚠️未消化的高优先风控依据」），**优先级顺序未动**。选项：①风控恒定优先 ②按依据新鲜度决定 ③取两者更严者 |
