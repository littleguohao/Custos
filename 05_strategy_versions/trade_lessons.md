...
> | `00_governance/research/` | 回测**假设检验**（历史数据上的因子/策略研究，与实盘账户行为无关） |
>
> ⚠️ **只增不删。** 结论被后续证据推翻时，把状态改成 `已推翻` 并写清推翻依据，
> 不要删行 —— 与 `00_governance/research/` 同一约定。删掉之后就没人记得
...
**证据必须可追溯。** 机器已经在产出结构化归因，直接引它：
    ...
    04_reviews/weekly/{iso_year}W{week}_weekly_review.json   ← weekly_review.py 产出
...
`00_governance/strategy/_shared/system_principles.md`）。原文只写了结论，
