...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R34["R34 打分系统进化v1<br/>🔄 来源一结局②(C4筛选假象嫌疑)<br/>来源二跑数中<br/>新轴基因组×权重格"]
    end
...
| [R34](R34_score_system_evolution.md) | 打分系统进化 v1：新轴基因组（腿集合×权重格）裁决 | 🔄 首批已判·来源一 r34_v1 **结局②否决**（C4：top objective −0.0007 打不过随机臂最佳 +0.5474 ⇒ 筛选假象嫌疑；判定窗再衰减至 −0.4746；C1 过/C2 等权基准编译越界读数缺失/C3 零翻转/C5 未启动）；来源二 r34_v2（score_legs_r1 池 6 腿）跑数中；判据 C1~C5 跑数前写死（C1 单格≥100 笔 / C2 top 基因组 Δmargin>0 双窗同向 / C3 ±50% 灵敏度零翻转 / C4 打过随机臂最高分 / C5 pre2019 终审一票否决）；核心洞察：复合打分编译成单条 DSL 表达式（TS_RANK 归一加权和）复用 strategy_grid 全部机制；工具 score_evolution_study + score_genome（v0.230 落地，v0.233 修生产机 MAX_PATH 写盘坑） | L3− | 🔄 跑数中·首批已判（2026-09-15 来源一结局②）|
