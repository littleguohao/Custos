...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R21["R21 深水RSI gate 验证<br/>✅ 跨窗通过（四档压基底）<br/>⚠️ 富集≠可交易（rsi_div）<br/>📌 live 接线=门内提醒（v0.229）"]
        ...
        R33["R33 QSX排除态可移植回避<br/>❌ 否决（kill-switch第0步<br/>排除边际-12.4pp胜率<br/>砍掉的是好交易）"]
        R34["R34 打分系统进化v1<br/>❌ 证伪收口（双来源结局②<br/>C4打不过随机臂+0.5474）<br/>新轴路线同死 转元层反思"]
        R36["R36 完美B1正例监督式打分<br/>🔄 Phase1✅/Phase3调权格❌C4<br/>Phase2三轮0pass挖掘路收敛到负<br/>近失候选复核：旧门没误杀"]
    end
    ...
    subgraph TRD["交易管理 —— 当前主战场"]
        ...
        R37["R37 出场轴进化战役<br/>🔄 首批 candidate_found<br/>出场基因组×战役壳<br/>双向停止+台账记账"]
    end
    subgraph META["元层 —— 决定上面能不能信"]
        R12["R12 判据演化<br/>6 类口径错误"]
        R13["R13 可复现性<br/>宇宙/窗口漂移"]
        R15["R15 偏差与边界"]
        R32["R32 进化引擎冒烟<br/>✅ 全闭环（#69/#70/#71/#75）<br/>副产：Amihud 选中子集最优"]
        R35["R35 打分系统存在性检讨<br/>📋 开题（元层反思）<br/>五轮证伪总账在案<br/>三终局候选待拍板"]
    end
...
| [R21](R21_rsi_deep_gate_validation.md) | 深水RSI 进场 gate 验证 | ✅ 跨窗通过：四档出场压基底（margin +26~+42pp，Wilson 不重叠）；12% 止损跨窗跳升最优；rsi_div 富集≠可交易 | L3 | ✅ 跨窗通过；📌 live 接线已落地（2026-09-14，v0.229）：owner 拍板=门内提醒（行动区 ⚡RD 仅提示），rsi_state→candidate，止损联动不启动 |
...
| [R32](R32_evolution_engine_smoke.md) | LLM 进化引擎真实数据冒烟 + 三轮修复验证 + 随机 baseline 裁决 | ✅ 全部闭环：链路三通（DSL 门 24/24）；#69/#70/#75 修复均生产机复测——r4 同 gate/出场两格 objective 分化（scorer 轴恢复）、慢格 18s/格且修复前后逐位一致；#71 裁决：随机过门 17% ≪ LLM 67%/100%（Fisher p=0.057）⇒ LLM 假设生成有增量（初步）；副产：纯 Amihud 选中子集最优（收益/回撤 5.73，R11 caveat）；VWAP 候选三轴 best cell 0.7395（线索留证） | L4 / L3− | ✅ 闭环（2026-09-12）/ 候选晋级走注册表流程 + 增量腿检验 |
| [R33](R33_qsx_exclusion_portable_filter.md) | 「跌破 QSX/DKX 未收复=排除」独立成可移植回避条件的边际验证 | ❌ **否决收口**（2026-09-16，kill-switch 第 0 步终止）：Cp 臂 ±排除态差值法（BBI 族口径，2010 起 s3000 终审）——排除项边际全指标为负：胜率 24.4% vs 36.8%（−12.4pp，Wilson 不重叠）、均收 −0.48pp、期望R −0.040；400 冒烟同号（−12.3pp）⇒ ≤0 实锤，按手册直接结局②，gate 工程与四步双窗免跑；「可移植风控腿」建议回收：排除态只在共振命中票内、负基线出场族下成立（与 R23 v2 BBI 先验互洽——正基线下它砍掉的是好交易） | L3− | ❌ 否决（2026-09-16 kill-switch 签发）|
| [R34](R34_score_system_evolution.md) | 打分系统进化 v1：新轴基因组（腿集合×权重格）裁决 | ❌ **证伪收口**：双来源同判结局②——r34_v1（smoke_r3_joint 池）top objective −0.0007 / r34_v2（score_legs_r1 池）top −0.0039，均打不过随机臂最佳 +0.5474 ⇒ C4 筛选假象嫌疑×2；判定窗再衰减（−0.4746 / −0.7970）；C1 均过 / C2 等权基准编译越界缺失（v0.234 已勘误，口径事故非证据）/ C3 均零翻转 / C5 未启动；补跑决议=不补跑（C4 独立死刑）；R22-R30 旧轴调权四轮之后「增量在新轴」路线同死，打分系五轮重建收口转元层反思；判据 C1~C5 跑数前写死；工具 score_evolution_study + score_genome | L3− | ❌ 证伪（2026-09-15 双来源判毕）|
| [R35](R35_score_system_existence_review.md) | 打分系统存在性检讨（元层反思开题） | 📋 开题：五轮证伪总账在案（R22 变体/R24 校准/R29 稳定/R30 组合搜索旧轴调权四轮 + R34 新轴基因组双来源 C4 打不过随机臂 +0.5474）；三终局候选——A 维持 V0 定性粗过滤 / B 简化腿数只留有独立证据的腿（rsi_deep 唯一跨窗证据）/ C 撤分改门（打分退役，gate 布尔+分位数分层）；决策路径与 owner 问题清单已列；非目标：不开第六轮寻优、不动 live 现行 | L4 | 📋 开题（2026-09-16 落档，待 owner 拍板）|
| [R36](R36_perfect_b1_supervised_scoring.md) | 完美 B1 正例监督式打分研究 | 🔄 进行中：Phase 0 ✅ 审计落档；**Phase 1 ✅ 双口径诊断回填**（live V0 把 8/10 买点落全宇宙前 20%、漏 300689 罚分误伤型/605378 无深J平区型；研究侧 reversal_quality 唯一落点高∧ICIR 正双占，kdj_j 买点落最低 14% 分位与 j_low 主轴互洽）；**Phase 2** 一轮 15 候选 IC 门全灭、二轮 1 pass 独苗终审**结局②否决**（截面 IC 双窗正但交易层 margin −2.8pp/−14.5pp 双窗负；C4 过=非筛选假象）；**Phase 2 三轮判毕**（v0.252 门改造首用例：10 候选 0 pass——6 条 marks 门死免 IC 评估（成本控制生效，token −70%）、3 条 marks 高分 0.75~0.875 全灭于 top_tail 贴线、1 条无值）；门次序复核终裁：近失候选（marks 0.83）三门对照 marks✅/rank❌/top_tail❌ ⇒ **旧门没误杀**——自指分离度≠宇宙级 top_tail 边际，「换门救候选」期望落空；Phase 2 挖掘路三轮收敛到负（r1 IC 门全灭 / r2 独苗交易层负 / r3 marks 高分过不了 top_tail）；**Phase 3 🔄 C4 复核中**（v0-lattice 满配：top=单腿 adx_gt_60，C1/C2/C3 过、C4-only 死刑随 v0.266 口径勘误转复核——owner：判定窗 +0.1347 真样本外 ~3.3 SE 且窗间增强+机制自洽；「配比无增量」半句成立，「单腿也没价值」推论 100% 靠 C4；N=50 修复后采样器 q95 机械判定线预注册 v0.268，过线走 C5 pre2019）；判据 R36-C1~C5 不变（正例召回率永远不作判据）；验证宇宙剔除 10 案例股 | L1 样本 / L3− | 🔄 进行中（2026-09-23：#78 满编 47/47 挖掘路证伪闭环；**思路二判毕 4 候选全死**——top argmax 直接落无腿基准，12 加腿格挖掘窗 Δmargin −0.15~−0.25 全负（以上死于 C2/C3 成立）；**Phase 3 top adx_gt_60 C4 死刑转复核🔄**（v0.268 预注册，判定窗真样本外增强在案）；#77 收口拍板待 C4 复核+签发）|
| [R37](R37_exit_axis_evolution_campaign.md) | 出场轴进化战役（出场基因组 × 战役壳 runner） | 📋 预注册落档：入场钉死 V0+j_low 基底只动出场（问题隔离），基因组=机制开关×参数档（simulate_b1_trade 现有参数空间，不引入新机制）；战役壳=外层确定性控制器 CTL-1~5（继续/转向/双向停止/预算帽 500）+台账多重比较记账；判据 C1~C5 同族（基准档 pct5_trail08，随机出场臂标尺自备不沿用入场 +0.5476）；审计在案：回测↔live 两侧规则集不对称（breakeven/trailing/time_stop/cost_zone 回流从未发生；N 型破位等 live 信号从未回测化=R38 立项候选）；非目标：不动 live/不做仓位轴/不重开入场侧 | L2 起步 / L3− | 🔄 首批战役判毕 candidate_found（2026-09-23，r37_b1 15 批 360/500：候选 sp8|cost_zone=3x2|time_stop=20|trail=0.08 C1~C4 全过 confirmed，判定窗 Δmargin +0.0034 贴零注记；qsx 家族批 13 连死关闭与 R33 互证）——C5 pre2019 终审待 owner 拍板 |
...
- **LLM 进化引擎口径（2026-09-09）**：`research/evolution_loop.py`
  ...
  seed 钉窄）。#75 慢格已修复并复测闭环（v0.212：TS_RANK 向量化 +
  表达式 scorer 预计算旁路，约 12000 倍；2026-09-12 生产机复测
  4 格 ×18s，expr3 修复前后读数逐位一致；4 scorer 选中子集真实分化，
  纯 Amihud 组合读数最优——top_n=0 退化时代不可见）。
