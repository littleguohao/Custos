...
flowchart TB
    ...
    subgraph META["元层 —— 决定上面能不能信"]
        ...
        R15["R15 偏差与边界"]
        R32["R32 进化引擎冒烟<br/>✅ 链路三通+修复验证<br/>⚠️ top_n=0 因子轴退化"]
    end
...
| [R32](R32_evolution_engine_smoke.md) | LLM 进化引擎真实数据冒烟（mock/双窗/joint 三步 + 修复后重跑） | ✅ 链路三通（DSL 门 18/18、双窗终审 r1 4/4 + r3 5/5）；#69 闭环（v0.207 透传 `--count`，r3 重跑 grid-judge 实跑 18 格）；VWAP 候选三轴 best cell 0.7395（j_low_adx25×base_low，R11 caveat）；⚠️ 实锤 `top_n=0` 下 scorer 轴不写交易集（哈希一致）——因子轴退化（TODO #70）；LLM 增量无对照，裁决实验 TODO #71 | L4 / L3− | ✅ 冒烟+修复验证（2026-09-11）/ ⚠️ 候选仅线索，晋级须增量检验 |
