...
flowchart TB
    ...
    subgraph TRD["交易管理 —— 当前主战场"]
        ...
        R37["R37 出场轴进化战役<br/>📋 预注册落档<br/>出场基因组×战役壳<br/>双向停止+台账记账"]
    end
...
| [R37](R37_exit_axis_evolution_campaign.md) | 出场轴进化战役（出场基因组 × 战役壳 runner） | 📋 预注册落档：入场钉死 V0+j_low 基底只动出场（问题隔离），基因组=机制开关×参数档（simulate_b1_trade 现有参数空间，不引入新机制）；战役壳=外层确定性控制器 CTL-1~5（继续/转向/双向停止/预算帽 500）+台账多重比较记账；判据 C1~C5 同族（基准档 pct5_trail08，随机出场臂标尺自备不沿用入场 +0.5476）；审计在案：回测↔live 两侧规则集不对称（breakeven/trailing/time_stop/cost_zone 回流从未发生；N 型破位等 live 信号从未回测化=R38 立项候选）；非目标：不动 live/不做仓位轴/不重开入场侧 | L2 起步 / L3− | 📋 预注册落档 + Phase 1 工程落地（2026-09-23，v0.264——战役壳/基因组/台账钉测 +42），待生产机跑出场轴战役第一批（命令见 R37 附录）|
