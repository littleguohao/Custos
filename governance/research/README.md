...
flowchart TB
    ...
    subgraph META["元层 —— 决定上面能不能信"]
        ...
        R15["R15 偏差与边界"]
        R32["R32 进化引擎冒烟<br/>✅ 链路三通<br/>⚠️ grid-judge 缺 --count"]
    end
...
| [R32](R32_evolution_engine_smoke.md) | LLM 进化引擎真实数据冒烟（mock/双窗/joint 三步） | ✅ 链路三通：DSL 门 6/6×2、双窗终审 5/5、cell 签名复用正常；joint 首候选 VWAP偏离×量比双窗 pass（线索留证）；⚠️ `--grid-judge` 3 格全灭根因=`_grid_command` 未转发 `--count`（尾部截断护栏按设计 fail-closed），修复+重跑见 TODO #69 | L4 / L3− | ✅ 链路验证（2026-09-10）/ ⚠️ 三轴终审待 #69 重跑 |
...
- **LLM 进化引擎口径（2026-09-09）**：`research/evolution_loop.py`
  ...
  （DSL 门 6/6×2、双窗终审 5/5）；发现 `--grid-judge` 子进程未转发
  `--count`（默认 500 尾部≈2024-08 触发截断护栏，3 格全灭），
  修复与重跑登记 TODO #69。
