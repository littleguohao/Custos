...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R30["R30 权重有界组合搜索<br/>📋 预注册（5702 组合）<br/>加严线 45%/2.6+零翻转<br/>⚠️ 搜索族证据封顶 L3−"]
        R31["R31 QN 因子批 8 gate<br/>📋 预注册（骑牛体系首批）<br/>双窗 H20>+3pp 且同向<br/>⚠️ 多重比较税：过线只记线索"]
    end
...
| [R30](R30_score_combo_search.md) | 打分权重有界组合搜索 | 📋 预注册：5 正腿×{0,10,20,30,40}×负腿块二态 = 5702 组合（gcd 排序等价去重）；调参双窗加严线 45%/2.6 + ±50% 零翻转（显著性税），幸存者按两窗 min(margin−全样本) 取 top 3 进 pre2019 终审（R29 基线 40%/2.4/半窗）；工程载体 score_combo_search_study.py（fast path 与慢路径逐位对拍钉测） | L3− | 📋 预注册（2026-09-08，跑数前）|
| [R31](R31_qn_factor_validation.md) | QN 因子批（骑牛登山体系 8 gate）入场加值验证 | 📋 预注册：8 gate（MA25 状态/倍量切/三线红/买小绿/J 负值计数/ADX 极端位/1.3 箱体/144 四要素）× 统一出场轴 × 双窗 s3000；判据 C1 单窗 ≥100 笔、C2 H20 加值 >+3pp 且双窗同向、C3 盈亏比 ≥基准 80%、C4 单 gate 过线只记线索（多重比较税）；因子 untested/none/debug，不进 live | L3− | 📋 预注册（2026-09-08，跑数前）|
