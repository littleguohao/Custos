...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R34["R34 打分系统进化v1<br/>📋 预注册待跑数<br/>新轴基因组×权重格<br/>增量在新轴不在调权"]
    end
...
| [R34](R34_score_system_evolution.md) | 打分系统进化 v1：新轴基因组（腿集合×权重格）裁决 | 📋 预注册待跑数：判据 C1~C5 跑数前写死（C1 单格≥100 笔 / C2 top 基因组 Δmargin>0 双窗同向 / C3 ±50% 灵敏度零翻转 / C4 打过随机臂最高分 / C5 pre2019 终审一票否决）；核心洞察：复合打分编译成单条 DSL 表达式（TS_RANK 归一加权和）复用 strategy_grid 全部机制；工具 score_evolution_study + score_genome 已落地（v0.230，钉测 49 例）；对照臂：等权/单腿/随机腿/s_shape，V0 臂 deferred（非 DSL 可表达） | L3− | 📋 预注册（2026-09-14 落档，v0.230 工具已落地）|
