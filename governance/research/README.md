...
| [R37](R37_exit_axis_evolution_campaign.md) | 出场轴进化战役（出场基因组 × 战役壳 runner） | 📋 预注册落档：入场钉死 V0+j_low 基底只动出场（问题隔离），基因组=机制开关×参数档（simulate_b1_trade 现有参数空间，不引入新机制）；战役壳=外层确定性控制器 CTL-1~5（继续/转向/双向停止/预算帽 500）+台账多重比较记账；判据 C1~C5 同族（基准档 pct5_trail08，随机出场臂标尺自备不沿用入场 +0.5476）；审计在案：回测↔live 两侧规则集不对称（breakeven/trailing/time_stop/cost_zone 回流从未发生；N 型破位等 live 信号从未回测化=R38 立项候选）；非目标：不动 live/不做仓位轴/不重开入场侧 | L2 起步 / L3− | 🔄 首个候选 C5 终审未过（2026-09-24 killed：a_sample n_taken=21<100 + 符号条款同成立 Δmargin −0.2396（−3.25·SE，CI95 全负）；首批战役 candidate_found（15 批 360/500），判据族+战役壳全链沉淀）——下一候选或收口待 owner 拍板 |
