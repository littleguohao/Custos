...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R36["R36 完美B1正例监督式打分<br/>🔄 Phase1✅诊断回填<br/>Phase2独苗终审否决<br/>Phase3调权格❌C4suspect"]
    end
...
| [R36](R36_perfect_b1_supervised_scoring.md) | 完美 B1 正例监督式打分研究 | 🔄 进行中：Phase 0 ✅ 审计落档；**Phase 1 ✅ 双口径诊断回填**（live V0 把 8/10 买点落全宇宙前 20%、漏 300689 罚分误伤型/605378 无深J平区型；研究侧 reversal_quality 唯一落点高∧ICIR 正双占，kdj_j 买点落最低 14% 分位与 j_low 主轴互洽）；**Phase 2** 一轮 15 候选 IC 门全灭、二轮 1 pass 独苗终审**结局②否决**（截面 IC 双窗正但交易层 margin −2.8pp/−14.5pp 双窗负；C4 过=非筛选假象）；门次序复核实证：IC-first 误杀 marks 0.83 高分候选（观测性留痕）；**Phase 3 ❌ 结局②否决**（v0-lattice 满配：top=单腿 adx_gt_60，C1/C2/C3 过、C4 打不过随机臂 +0.5476——调权路在 V0 腿上同样死刑，R30/R34 五轮档案互证；随机天花板 +0.5474→+0.5476 跨批精确复现）；判据 R36-C1~C5 不变（正例召回率永远不作判据）；验证宇宙剔除 10 案例股 | L1 样本 / L3− | 🔄 进行中（2026-09-18：Phase 2 三轮门指标改造预注册落档——top_tail 头部价差门 + marks_first 次序，owner 拍板思路一；待生产机跑 r36_p2_r3）|
