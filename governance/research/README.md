...
flowchart TB
    ...
    subgraph SEL["选股方向 —— 已关闭"]
        ...
        R33["R33 QSX排除态可移植回避<br/>📋 预注册待跑数<br/>跌破未收复=排除独立成腿<br/>边际正/负？"]
...
| [R33](R33_qsx_exclusion_portable_filter.md) | 「跌破 QSX/DKX 未收复=排除」独立成可移植回避条件的边际验证 | 📋 预注册待跑数：判据 C1~C4 跑数前写死（C1 单格≥100 笔 / C2 胜率差 +3pp 双窗同向 / C3 盈亏比≥基准 80% / C4 单格只记线索）；先验双面——R23 v2 排除项 QSX 族掰正负基线、BBI 族拖低正基线，加值依赖出场族；工程载体评估：现 qsx gate 是结构过滤不表达排除态，前置最小 qsx_exclusion_free gate 口径已写死（owner 拍板后实施 <1h）；生产机手册四步（kill-switch 差值法→gate 工程→双窗 s3000→digest 核对） | L3− | 📋 预注册（2026-09-14 落档，v0.228）|
