...
> **状态**：📋 **预注册（2026-09-07，跑数前）**——判据与候选本页写死，跑数后如实回填
...
> 产物：`artifacts/logs/score_stability_study/r29_phase2.json`、
> `r29_phase3_pre2019.json`（跑数后落）。
...
📋 **预注册（2026-09-07，跑数前）：结论未定且不许预设。** 判定是机械的：
Phase 2（主窗+跨窗）四候选中两窗全过且参数不敏感者进终审；pre2019 终审
R29-C1∧C2∧C3 全保持 ⇒ 判过进 Phase 4（live 落地）；任一失线 ⇒ 判负——
与 R22/R24 同款结局也是合格产出，届时如实回填本段并更新头部状态，权重不进
live。前两轮的教训（R22 V2 窗内 46~54% 胜率照翻）说明：窗内达标不构成任何
乐观先验，本页不给预期方向。
...
（待 Phase 2/3 跑数后如实回填：候选三窗读数、灵敏度、终审判定、结论与状态。）
