...
> **状态**：❌ **证伪收口（2026-09-15 双来源判毕）**——2026-09-14 落档预注册，
> v0.230 工具落地；v0.231 增强（两阶段省钱 + V0 臂）；v0.233 修 MAX_PATH；
> v0.234 复合编译口径勘误 + s_shape 根因处置——**来源一 r34_v1 与来源二
> r34_v2 均判结局②否决**（C4 打不过随机臂，同标「筛选假象嫌疑」），
> 打分进化 v1 证伪，打分系收口转元层反思；判据 C1~C5 与窗口/宇宙/出场轴/
> 对照臂已全部写死；裁决驱动
...
**❌ 打分系统进化 v1 证伪收口（2026-09-15，双来源同判结局②否决）。** 两个
独立腿集合的新轴基因组都打不过随机腿复合同待遇臂——#71 纪律的核心问题
「新轴本身有没有加值」，答案是**没有**：

- **来源一 r34_v1**（smoke_r3_joint 冒烟池 6 腿）：top w=[0,0,1,0,0,2]
  挖掘窗 objective −0.0007 vs 随机臂最佳 +0.5474 ⇒ C4 suspect；判定窗
  再衰减（−0.4746 / margin −0.130 / wr 27.9%）。C1 过（62010/45314 笔）；
  C2 等权基准编译越界读数缺失（口径事故非证据，v0.234 已勘误）；C3 零
  翻转；C5 按纪律未启动。
- **来源二 r34_v2**（score_legs_r1 池 6 腿，LLM 攒「打分轴」方向新腿）：
  top w=[0,0,0,0,3,1] 挖掘窗 objective −0.0039 vs 同一随机臂最佳 +0.5474
  ⇒ C4 suspect；判定窗衰减更惨（−0.7970 / wr 19.7%）。C1 过
  （37069/26660 笔）；C2 同款缺口（父进程早于 v0.234，同跑旧口径）；
  C3 零翻转；C5 未启动。
- **补跑决议：不补跑。** 两轮死刑均由 C4 独立签发（随机臂 +0.5474 是不可
  逾越线），v0.234 只修复 C2 可算性、改变不了任何一轮结局；有效的等权
  基线读数留给下一代打分研究。

R22 变体 / R24 校准 / R29 稳定 / R30 组合搜索四轮旧轴调权全证伪之后，
「增量在新轴」这最后一条辩护路线也死了——打分系重建五轮收口，
转元层反思（打分系统的存在性检讨，而非第六轮参数/轴寻优）。
...
- **R34-C4（随机对照）**：top 基因组 objective 必须**打过随机臂最高分**
  ...
  新轴增量不成立，直接结局②
- ⚠️ **C4 口径勘误（2026-09-23，v0.266，owner 方法论 review）**：本单元
  两轮死刑均由 C4 独立签发，而该对照臂是**固定种子单次抽样**
  （「+0.5474 复现」=同种子重言式）且含**裸 close/volume 终结符**（真
  因子代理）——**不是干净零假设**，本单元结局②的证据等级随之下调；
  辅证在案：判定窗衰减 −0.4746/−0.7970。按新 95% 分位口径（R37-C4）
  复核与否 owner 拍板（TODO #80）；R36 不受影响部分见其判据节勘误。
  **2026-09-23 v0.268 进展**：复核协议已在 R36 Phase 3 落档（N=50 修复后
  采样器 × 同 64 格，q95 机械判定线）；**本单元两轮 top 暂缓复核**——
  负 objective（−0.0007/−0.0039）即使分位下移翻转概率也低 + 判定窗
  衰减辅证，待 R36-P3 复核读数（q95 若 < 0 再议）。
...
**+0.5474**。
**来源二 r34_v2 读数**（落盘点
`artifacts/logs/score_evolution/r34_v2/_score_evolution__r34_v2.json`）：
top = w[0,0,0,0,3,1]（3×腿4 + 1×腿5，量能族组合），挖掘窗 objective=−0.0039 /
margin=−0.0174 / wr=33.7% / payoff=1.819 / n=37069（格 sig ed72eff8792f）；
判定窗 objective=−0.7970 / margin=−0.2040 / wr=19.7% / n=26660——双窗衰减
比 r34_v1 更惨。等权基准同款编译越界缺失（symbol_len=457>300、depth=15>12：
r34_v2 父进程 2026-09-14 21:35 启动、早于 v0.234 落地，全程跑在旧口径，
勘误段「含 r34_v2 的 C2 读数有效」的预期不适用于本轮）；随机臂同种子
同池，best 同为 −0.0987 / −0.0143 / **+0.5474**。
③ **C1~C4 逐条读数与判定**：
- r34_v1：C1 ✅（62010/45314 笔 ≥ 100）；C2 ⚠️ 读数缺失（等权基准缺，
  按保守口径不计过）；C3 ✅ 翻转 0/4（已无意义）；C4 ❌ **suspect**
  （−0.0007 < +0.5474）⇒ **结局②否决**。
- r34_v2：C1 ✅（37069/26660 笔 ≥ 100）；C2 ⚠️ 同款缺失；C3 ✅ 翻转 0/4；
  C4 ❌ **suspect**（−0.0039 < +0.5474）⇒ **结局②否决**。
- **补跑决议：不补跑**（回应勘误段「补跑决议见回填区」）——两轮死刑均
  由 C4 独立签发，v0.234 只修复 C2 可算性、改变不了任何一轮结局；
  打分进化 v1 证伪收口。
④ **C5 终审读数**：not_run——两轮 C4 均未过按纪律止步，pre2019 终审段
untouched。
⑤ **腿集合来源**：来源一 r34_v1 =
`artifacts/logs/evolution/smoke_r3_joint/trajectory_pool.json`（自动取
pass∧gate非空 6 条，2018-2020 冒烟双窗 pass 者，VWAP 偏离×量能族）。来源二
r34_v2（已判，见 ②③）= `score_legs_r1` 池（方向「分层打分新轴：超卖
