...
回答 owner 的连环问句并留证：① RS 配 RD 的优胜出场组合（pct12 + 分批止盈
+ BBI 跌破两根）是否翻盘；② RSI 家族（RS/RD/R★/RV）与形态家族（B2/异动后B1/
突破回踩B1/主升始发点）在同一出场轴上的双窗（跨窗 2022-2024、主窗
2024-08~2026-09）margin/胜率/盈亏比读数；③ 为 1800 标注一览的「盈亏比/胜率」
列提供同口径数据源。
...
**④b QG（共振v2，为 1800 一览「盈亏比/胜率」列补同口径数据）**：跨窗
pct12_so5_bbi2 档 31.4% / 2.73 / +4.6pp，主窗 35.8% / 2.60 / +8.0pp——
两窗都只有基底同档（+15.0/+20.2pp）的一半以下，与 R23（计数零筛选力）、
R26（负边际）一致；高盈亏比来自低频长持，不代表选股能力。
（产物：`_ranked__qg_{cw,main}.json`，j_low 对照格复用本批既有结果。）
