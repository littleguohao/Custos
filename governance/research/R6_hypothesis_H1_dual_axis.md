...
> 注：下文因子路径 `screening/b1_dual_factor.py` 已迁至 `src/custos/core/factors/`。
...
uv run python src/custos/research/backtest_factors.py --scorer b1_dual   --entry-filter j_low --horizons 5,20,60
uv run python src/custos/research/backtest_factors.py --scorer s_shape   --entry-filter j_low --horizons 5,20,60
uv run python src/custos/research/backtest_factors.py --scorer long_structure --entry-filter j_low --horizons 5,20,60
uv run python src/custos/research/backtest_factors.py --scorer s_reversal --entry-filter j_low --horizons 5,20,60

# ② 门槛对比：J<13 单独 vs 叠加 QSX>DKS vs 突破回踩型
uv run python src/custos/research/backtest_factors.py --entry-filter j_low
uv run python src/custos/research/backtest_factors.py --entry-filter j_low_qsx_gt_dks
uv run python src/custos/research/backtest_factors.py --entry-filter breakout_pullback_b1

# ③ 日周共振（H1c）：共振加分是否有增益 / 共振与结构完好哪个胜率高
uv run python src/custos/research/backtest_factors.py --scorer b1_dual        --entry-filter j_low --horizons 5,20,60
uv run python src/custos/research/backtest_factors.py --scorer b1_dual_no_res --entry-filter j_low --horizons 5,20,60
uv run python src/custos/research/backtest_factors.py --entry-filter weekly_j_low
uv run python src/custos/research/backtest_factors.py --entry-filter j_low_weekly_resonance
uv run python src/custos/research/backtest_factors.py --entry-filter j_low_qsx_weekly   # 最严一档，看召回代价

# ④ 净值终审（跨窗，必须赢过无条件基准）
uv run python src/custos/research/backtest_factors.py --trade-sim --scorer b1_dual --entry-filter j_low_qsx_gt_dks --cost-bps 25
```
...
uv run python src/custos/research/backtest_factors.py --scorer b1_dual --entry-filter weekly_j_low            --universe-local --universe-sample 1000
uv run python src/custos/research/backtest_factors.py --scorer b1_dual --entry-filter j_low_weekly_resonance  --universe-local --universe-sample 1000
uv run python src/custos/research/backtest_factors.py --scorer b1_dual --entry-filter j_low_qsx_weekly        --universe-local --universe-sample 1000
```
...
数据：`artifacts/logs/backtest_baseline_none_1000.json`、`backtest_h1_weekly_j_low_b1dual_1000.json`、
...
uv run python src/custos/research/backtest_factors.py --scorer b1_dual --entry-filter j_low_qsx_weekly --universe-local --universe-sample 1000 --seed 1 --horizons 5,10,20,60
# ⚠️ 跨区间必须加大 --count:count 默认 500 根从今天往前数,加 --start/--end 只覆盖
#    窗口尾部(实测只剩 2024H2);--count 1500 才能覆盖 2021 年初的预热段
uv run python src/custos/research/backtest_factors.py --scorer b1_dual --entry-filter j_low_qsx_weekly --universe-local --universe-sample 1000 --start 2022-01-01 --end 2024-12-31 --count 1500 --horizons 5,10,20,60
# 净值终审（跨窗必须赢无条件基准）
uv run python src/custos/research/backtest_factors.py --trade-sim --scorer b1_dual --entry-filter j_low_qsx_weekly --cost-bps 25 --universe-local --universe-sample 1000
```
...
数据：`artifacts/logs/backtest_*_{seed1,2022_2024,tradesim}*.json`。
