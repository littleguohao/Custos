...
> **状态**：❌ **否决收口（2026-09-16，kill-switch 第 0 步终止）**——排除项
> 边际在 3000 只终审口径下全指标为负（胜率 −12.4pp），按手册「≤0 直接
> 结局②」签发否决，gate 工程与四步主研究免跑；判据 C1~C4 与窗口/宇宙/
> 出场轴/工程载体口径已全部写死（正文备查）；本机无通达信数据，跑数走附录生产机手册　|
...
**❌ 否决收口（2026-09-16，kill-switch 第 0 步终止，结局②）。** kill-switch
差值法（Cp 共振独立臂含排除态 vs 同臂 `--no-exclusion`，BBI 族出场口径，
2010 起 s3000 终审样本）：排除项边际**全指标为负**——胜率 24.4% vs 36.8%
（**−12.4pp**，Wilson 区间 [0.2367,0.2513] vs [0.3646,0.3709] 不重叠）、
均收 +0.43% vs +0.91%（−0.48pp）、期望R +0.036 vs +0.076（−0.040）；
400 只冒烟先行同号（胜率差 −12.3pp），扩样后纹丝不动。按手册「排除项
边际 ≤0 ⇒ 直接结局②否决」签发：**「跌破 QSX/DKX 未收复」作为独立可移植
回避条件证伪**，gate 工程（qsx_exclusion_free）与四步双窗主研究按手册
省掉。口径警示照录：差值法测的是「共振命中票内」的排除边际——它回答
「排除态在这批票里砍掉的到底是好交易还是坏交易」，答案是砍掉的是好交易
（留下的反而更差）；与 R23 v2 的 BBI 族先验（排除提升期望R +0.026→+0.053）
方向相反——那时排除是救负基线的腿，如今在 Cp 正基线臂内它拖住收益
（与先验表「BBI 族正基线下 C 完整回不到基底 A」同向，方向性证据互洽）。
R23 v2 的「可移植风控腿」建议按结局②条款回收：排除态只在共振命中票内、
特定出场族语境下成立，不具备独立可移植性。

~~📋 预注册，尚无结论~~ 🔄（原预注册段保留备查，2026-09-14 落档时判据已写死）：
本页按 R12
...
④ **第 0 步 kill-switch 差值读数（2026-09-16 生产机；Cp 共振独立臂 ±排除态，
BBI 族出场口径 `--bbi-exit-consec 2 --qsx-exit-consec 0`，2010-01-01 起，seed=0）**：

| 臂 | 样本 | 笔数 | 胜率（Wilson95） | 盈亏比 | 均收 | 期望R |
|---|---|---:|---:|---:|---:|---:|
| Cp 含排除态 | 400 冒烟 | 1878 | 24.1% [0.2219,0.2605] | 4.011 | +0.67% | +0.056 |
| Cp 去排除态 | 400 冒烟 | 12309 | 36.4% [0.3551,0.3721] | 2.314 | +1.00% | +0.084 |
| Cp 含排除态 | **3000 终审** | 13297 | 24.4% [0.2367,0.2513] | 3.62 | +0.43% | +0.036 |
| Cp 去排除态 | **3000 终审** | 89867 | 36.8% [0.3646,0.3709] | 2.226 | +0.91% | +0.076 |

排除项边际（含 − 去）：胜率 **−12.4pp**（终审；冒烟 −12.3pp 同号）/
均收 −0.48pp / 期望R −0.040 ⇒ **≤0 实锤，按手册直接结局②否决**。
半窗一致性：两臂前后半窗均各自同向（排除臂 22.5%→25.6%（400 口径）；
去排除臂 35.3%→37.4%），负边际不是窗效应。
产物：`artifacts/logs/qsx_resonance_study/r33_kill_Cp_{400,400_noex,
3000,3000_noex}.json` + 同名 .out/.err.log（400 冒烟与 3000 终审）。

①②③⑤ **免跑**（kill-switch 已签发否决，gate 工程与四步主研究按手册省掉）：
qsx_exclusion_free 未实施（口径段保留备查，「口径一经实施不因跑数结果
修改」——未实施则无口径债）；两窗 codes-file/digest 未生成；strategy_grid
无双窗产物。
