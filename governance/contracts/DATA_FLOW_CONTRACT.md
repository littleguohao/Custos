...
（全仓 `cooldown`/`冷却`/`blacklist` 零命中）。是否要真做冷却见 `TODO.md` #31。
...
7. 候选发现由每日选股 screening 链产出（`07_tools/screening/`：公式初筛 → 充实/模式识别 → 板块共振打分 → 备选表格，18:00 独立链 `run_1800.py` 运行，输出 `data/stock_pool/YYYY-MM-DD_stock_pool.json`）；`theme_tracker` 的 sector_state 是其板块输入；`tdx-wxd-a`、`tdx-wxd-bk` 暂不接入。
