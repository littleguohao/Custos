...
> `governance/contracts/` 存放**契约与运行时配置**：代码直接依赖的东西。
...
⚠️ **配置路径只在 `07_tools/paths.py` 定义一次**，模块不得自己拼 `"governance"`
...
- `MASTER_WORKFLOW §十二` 原挂着 8 条「当前需要调整的旧设计」。**待办不该埋在契约文档里** ——
  ...
  于 2026-08-09 落地：统一实现 `07_tools/report_audit.py`，四份正式报告头部注入可审计块。
