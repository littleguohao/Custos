...
```
全A股(~5500)
  → [1] 公式初筛  src/custos/pipeline/screening/formula_screen.py
        TQ formula_process_mul_xg 批量跑注册公式 → data/screening/{date}_formula_hits.json
  → [2] 充实+模式识别  src/custos/pipeline/screening/enrich_candidates.py
...
- 公式注册表：`governance/contracts/SCREEN_FORMULA_REGISTRY.json`。**只能引用客户端
...
- **自选池通道**（`manual_pools`）：用户在通达信客户端手工维护的备选池
  （如"震荡"），经 `src/custos/pipeline/screening/manual_pools.py` 读取
...
- 股票池：全 A 含北交所（2026-09-19 v0.257 owner 拍板放开；exclude_bj 开关关闭；ST、上市天数在 [2] 过滤）。
...
- **正交因子（方向A，非量价形态）**：
  ...
  - **财务维度（脚手架，默认关）**：`src/custos/pipeline/screening/financials.py` 用 mootdx Affair 财务表做
...
- `candidate_table.py` 渲染分组表格进 `artifacts/reports/daily/{date}/`，
  ...
  证据列；v0.89 起「门槛外观察区」改为「📌 门内提醒」节（v0.160 起：震荡池内 J≤13 即入节，不再强制异动强）。
- `daily_report.py` 在"主线、机会与风险"节内追加"公式选股备选池"小节，
  ...
  （ADX25/S**/资金意图/0AMV 状态）+ 回退旧期时打**过期警示**；排序语义
  如实标注「形态分层、未校准启发式、非 alpha 排序」。

## 策略对齐（B1/CZ）

2026-07-21 起，enrich 段新增对齐 `governance/strategy/b1/01_swing_rules.md`（B1）与
`governance/strategy/cz/01_cognition_framework.md`（CZ）的确定性检测器。全部只依赖本地 vipdoc
...
- **门内提醒**（v0.89，owner；取代 v0.51 的门槛外观察区）：J<13 硬门槛
  ...
  「📌 门内提醒」一节列**震荡池内**（POOL_ZHENDANG，v0.91 显式收回）票：
  日 J≤13（v0.160 起**不再要求异动强**，异动判据列降级为展示信息），
  按日 J 从小到大最多前 20 只（v0.90）——仅提醒，不改分层/排序。
...
所有开关集中在 `governance/contracts/SCREEN_FORMULA_REGISTRY.json`，默认值＝历史行为，
改动前同样遵循「先回测」原则。
...
（单线程阻塞，需重启客户端恢复，见 governance/data/TDX_LOCAL_INTERFACES.md「TQ 服务可被打挂」）。
