...
- **09:05 / 14:45 不启用**：0AMV 与市场宽度本就要等收盘，盘中 blocked 属正常；14:45 把门控结论写进 `artifacts/logs/{date}_1445_run_log.json`（`_gate_note`）留痕并降低报告内的权限文案。
...
- 产物契约：`src/custos/core/contracts.py` 的 `runtime_gate`（三个 `allow_*` 布尔是权限本身）
