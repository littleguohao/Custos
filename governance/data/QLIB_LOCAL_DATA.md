...
>
> `E:\S_DATA` 下的 qlib bundle / 单票 CSV，通过 `src/custos/datasource/s_data.py` 只读访问。
> **只服务研究与回测链，live 链不用。**
>
...
```bash
uv run python src/custos/research/reconcile_qfq.py --qlib-selfcheck 600519
```
...
uv run python src/custos/research/reconcile_qfq.py --detail 600519

# 复权约定探测：乘法 vs 加法，并列出每段的调整量 c
uv run python src/custos/research/reconcile_qfq.py --convention 600519

# bundle 实有字段（看有没有 factor 能还原）
uv run python src/custos/research/reconcile_qfq.py --qlib-fields 600519
```
...
```bash
uv run python src/custos/research/reconcile_qfq.py --gap-report
```
