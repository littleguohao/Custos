...
> **上下文**：**通用因子**（跨策略可复用）的接口治理　｜　**执行者**：脚本（迁移期）+ owner（裁决点）　｜　**状态**：📋 设计稿待拍板（v0.216，2026-09-12）
> **版本**：v0.216（2026-09-12 初稿）　｜　**代码依赖**：现状清点见 §2（只读，不动实现）
...
- 43+1（补登记的 qsx_resonance）个因子模块全部暴露规范入口；`signal_labels.py`
  与 `backtest_factors.py` 中无手写因子判据（grep 级钉测：无 `_sc_` 包装体、
  signal_labels 无内联阈值算式）；
- characterization 快照 + 逐位等价钉测全绿；注册表/分层/架构测试全绿且新增
  「规范入口唯一」测试（模块顶层无第二套公开入口）；
- 影子对照：1800 候选表标注列 ≥10 个交易日与旧口径逐位一致；
- 生产机回测清单（§4.4）全过：关键格子修复前后逐位一致 + 双窗读数不变；
- TODO #67 删除，本稿状态转「✅ 已收敛」并回填证据链接。
