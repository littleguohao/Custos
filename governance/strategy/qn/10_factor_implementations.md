...
| factor id | 名称 | 规则出处 | kind | 研究 gate | 核心判定 |
|---|---|---|---|---|---|
| `qn_ma_converge` | 均线收拢发散 | [01 §五](01_general.md) | state | 首次放量向上发散 | MA5/10/25 三线带宽 ≤5% 持续 10 根 → 扩张 + 多头排列 + 放量阳 + 144 走平上翘托底（v0.200 案例校准：144 不进带宽） |
...
因子落地后拿知识库**带日期的实盘案例**逐条对照（腾讯前复权日 K，as-of 复跑）：
- ✅ `qn_kdj_neg_day`：腾龙股份 2023-11 逐分复现（死叉 J=79.25 一致、负值第 3/5
  天命中 11-16/11-20）——**校准**：smooth 腿收窄为「死叉→首根 J<0 全阴」，
  负值区允许小阳（详见 [05](05_top_bottom_kdj.md) 校准注记）
- ✅ `qn_weekly180_setup`：华升股份 2019-03-20 命中（老师「过 3.25 买进翻五倍」
  那波）；海南矿业反例正确不命中
- ✅ `qn_ma_converge`：**校准**——带宽由四线改三线（144 是托底不是绳身，
  卫宁健康/天顺股份实证四线 5% 永不成立）；校准后卫宁 2019-08-15、
  天顺 2019-09-26 命中
- ⚠️ 口径边界：`qn_shrink_limit_up` 是**低位启动板**（贴均线+放量阴在前）；
  开开实业 2023-08 这类高位接力缩量板属「打板次日行为」（明确未因子化项）
- ⚠️ 紫光股份 2026-08-13 的「阳包阴」实为叉上阴/插上阴（高开实体在前阴上方），
  与 `qn_bullish_engulf` 的实体包覆定义不同形态，不命中属正确
- ❓ 未复现：万年青/光弘科技（疑为 60 分钟级案例）、泰达股份（数据无对应涨停）、
  利亚德 2013（qfq 除权日失真）、亚星锚链（年份推断错误）——案例日期多由
  ASR 转写推断，存疑者标「未复现」而非判因子错
- R31 跑数（v0.199）对应**校准前**语义；两因子的 needs_work 状态不变，
  盈利判定以复跑为准
