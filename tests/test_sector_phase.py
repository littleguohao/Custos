...
from custos.core.factors import sector_phase as sp
...
def _fixture_sector_names(monkeypatch):
    ...
    monkeypatch.setattr(
        "custos.datasource.local_tdx.tq_sector.load_sector_names",
        lambda path=None: {
            "880201": {"name": "测试概念A", "tdx_type": "4"},
...
def test_favorable_series_causal_and_gate(tmp_path):
    ...
    (tmp_path / "880201.SH.csv").write_text(
        "date,close\n" + "\n".join(f"{d},{c}" for d, c in zip(dates, up)),
        encoding="utf-8",
    )
    (tmp_path / "880900.SH.csv").write_text(
        "date,close\n" + "\n".join(f"{d},{c}" for d, c in zip(dates, down)),
        encoding="utf-8",
    )
    fav = sp.favorable_series(dates, up)
    assert fav[dates[-1]] is True and all(
        v in (True, False) for v in fav.values()
    )  # 因果布尔
    ...
    assert (
        resolve("600000")["favorable"] is True
        and resolve("600000")["available"] is True
    )
...
def test_favorable_series_no_lookahead():
    ...
    ext_dates = dates + [
        str(d)[:10] for d in pd.date_range("2022-06-27", periods=15, freq="B")
    ]
...
def test_phase_dirty_input_no_raise():
    ...
    r = sp.compute_sector_phase(
        ["10", "x", None, 10.5] + [10 + 0.1 * i for i in range(80)]
    )
