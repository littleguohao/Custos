...
def _prices(code, close_by_date):
    ...
    return {
        code: {
            "dates": dates,
            "close_by_date": {d: float(close_by_date[d]) for d in dates},
        }
    }
...
class ReboundTests(unittest.TestCase):
    ...
    def test_rebound_reduce_sells_20pct_on_rebound_days(self):
        ...
        expected_realized = (9.8 * 200 - 1.96 - 200 * 10.0) + (
            10.1 * 160 - 1.616 - 160 * 10.0
        )
...
class SpecialCategoryTests(unittest.TestCase):
    ...
    def test_drawdown(self):
        dd = bt.compute_drawdown([10, 100, 40, 120, 30], ["d1", "d2", "d3", "d4", "d5"])
