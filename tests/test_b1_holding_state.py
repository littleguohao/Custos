...
class B1HoldingStateTests(unittest.TestCase):
    def test_l1_hard_breach_has_p0_priority(self):
        row = {
            ...
            "above_bbi": True,
            "n_structure": {"available": True, "prior_low": 10, "pullback_low": 12},
            "price_volume": {"available": True, "two_medium_large_bull": True},
        }
        state = evaluate(row, "做多")
        self.assertEqual(
            (state["final_priority"], state["final_action"]),
            ("P0", "N型主结构清仓评估"),
        )
    ...
    def test_l2_failure_is_distinct_from_l1(self):
        ...
        self.assertEqual(
            (state["final_priority"], state["final_action"]), ("P1", "N型回踩失守评估")
        )
    ...
    def test_bear_rebound_reduces_and_never_allows_add(self):
        row = {
            ...
            "box20_position": "箱体上半区",
            "above_bbi": True,
            "n_structure": {"available": False},
            "price_volume": {"available": True, "change_pct": 2.0},
        }
    ...
    def test_bear_regime_reduce_top_priority_fires_without_other_signals(self):
        ...
        self.assertEqual(
            state["signals"][0]["signal"], "bear_regime_reduce_top_priority"
        )
        self.assertFalse(state["permissions"]["allow_add"])
    ...
    def test_bear_regime_rule_does_not_fire_outside_bear_regime(self):
        ...
        for regime in ("做多", "中性", "未知"):
            ...
            self.assertEqual(
                (state["final_priority"], state["final_action"]), ("P3", "条件持有")
            )
            self.assertNotIn(
                "bear_regime_reduce_top_priority",
                [x["signal"] for x in state["signals"]],
            )
    ...
    def test_p0_hard_risk_still_outranks_bear_regime_rule(self):
        ...
        self.assertIn(
            "bear_regime_reduce_top_priority", [x["signal"] for x in state["signals"]]
        )
    ...
    def test_two_bull_profit_take_is_below_hard_risk(self):
        row = {
            ...
            "above_bbi": True,
            "n_structure": {"available": True, "prior_low": 10, "pullback_low": 11},
            "price_volume": {"available": True, "two_medium_large_bull": True},
        }
        state = evaluate(row, "做多")
        self.assertEqual(
            (state["final_priority"], state["final_action"]), ("P2", "分批止盈")
        )
