...
class CloseReviewValidationTests(unittest.TestCase):
    def test_valid_snapshot_passes(self):
        self.assertEqual(
            validate_quote_snapshot("2026-07-15", POSITIONS, valid_snapshot()), []
        )
    ...
    def test_report_requires_current_quote_gate(self):
        ...
        errors = validate_report(
            ...
            POSITIONS,
            report,
            {"position_gate": {"quotes_current": False}},
        )
        self.assertEqual(
            errors, ["runtime gate does not confirm current holding quotes"]
        )
    ...
    def test_delivery_digest_is_bounded_and_complete(self):
        digest = build_delivery_digest(
            ...
            POSITIONS,
            {
                "600000": {
                    "price": 10.0,
                    "bbi": {"state": "当前价在2026-07-14 BBI上方"},
                    "n_structure": {"state": "N型前低 9.00"},
                }
            },
...
class RiskSourceDateTests(unittest.TestCase):
    ...
    def _write(self, day):
        (self.tmp / f"{day}_risk_decision.json").write_text(
            '{"stock_risks": []}', encoding="utf-8"
        )
