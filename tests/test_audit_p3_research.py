...
class TestGateDependencyFailureNotSilent:
    ...
    def test_platform_pullback_dependency_missing_is_counted(self, monkeypatch):
        bt.reset_gate_stats()
        monkeypatch.setitem(
            sys.modules, "custos.core.factors.platform_pullback", None
        )  # import 触发 ImportError
        df = _bars(n=90)
    ...
    def test_platform_pullback_genuine_miss_recorded_as_miss(self):
        ...
        st = bt.gate_stats().get("platform_pullback") or {}
        assert (
            st.get("miss", 0) >= 1 and not st.get("dep_missing") and not st.get("error")
        )
...
class TestSectorFeatureBuildFailureNotSilent:
    ...
    def test_main_reports_sector_feature_yield_when_data_present(
        ...
        assert any(
            "f_sector_favorable" in (d[2] if len(d) > 2 else {})
            for r in head["records"]
            for d in r["days"]
        )

    def test_main_refuses_sector_features_without_any_index_data(
        self, tmp_path, capsys
    ):
        members = tmp_path / "members.json"
...
class TestEmptyResultMustFail:
    ...
    def test_backtest_factors_no_bars_exits_nonzero_without_writing(self, tmp_path):
        ...
        rc = bt.main(
            ["--codes", "600000", "--horizons", "5", "--out", str(out)],
...
class TestFinancialReportStaleness:
    ...
    def test_staleness_cap_can_be_disabled(self):
        df = self._df("2019-12-31")
        r = fin_mod.financial_factor(
            "600000", df, self._colmap(df), as_of="2026-05-30", max_age_days=0
        )
    ...
    def test_missing_report_date_is_reported_not_assumed_fresh(self):
        ...
        r = fin_mod.financial_factor(
            "600000", df, fin_mod.auto_colmap(df.columns), as_of="2026-05-30"
        )
...
class TestTierYouVisibilitySemantics:
    ...
    def _idx(self):
        ...
        return {
            "600000": [
                ("2026-01-10", "2025-12-31", 1.0, 0.5, 8.0),
...
class TestCliGuards:
    ...
    def test_out_creates_parent_dirs_pass2(self, tmp_path):
        ...
        rc = lp.main(
            ["--from-firings", str(firings), "--discriminate", "--out", str(deep)]
        )
    ...
    def test_out_creates_parent_dirs_long_windows(self, tmp_path, monkeypatch):
        ...
        rc = lp.main(
            ["--list-long-windows", "--out", str(deep), "--min-window-days", "1"]
        )
        assert rc == 0 and deep.is_file()
