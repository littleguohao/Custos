...
def test_fund_flow_of_in_rank_and_sector():
    ff = {
        ...
        "sectors": [
            {"name": "半导体", "main_net_inflow": 5e8, "main_net_pct": 2.0},
...
def test_fund_flow_missing_degrades():
    assert (
        ec.fund_flow_of("600000", "半导体", {"available": False})["available"] is False
    )
    ...
    lvl, score, detail = sc.capital_intent_strength(
        {
            ...
            "fund_flow": {
                "available": True,
                "in_rank_positive": True,
                "sector_inflow_positive": False,
            },
        }
    )
    assert (
        detail["fund_flow_inflow"]["hit"] is True
        and detail["fund_flow_inflow"]["points"] == 2
    )
...
def test_low_liquidity_flag_default_no_cap():
    s = sc.score_candidate(
        _strong_cand(liquidity={"available": True, "avg_amount_yi": 0.1}),
        SECTOR,
        "做多",
    )
...
def test_low_liquidity_caps_when_enabled():
    s = sc.score_candidate(
        ...
        SECTOR,
        "做多",
        cap_rules={"liquidity_floor": True},
    )
...
def test_ample_liquidity_no_flag():
    s = sc.score_candidate(
        _strong_cand(liquidity={"available": True, "avg_amount_yi": 3.0}),
        SECTOR,
        "做多",
    )
