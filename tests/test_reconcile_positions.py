...
class TestReusesSingleSourceOfTruth:
    def test_replay_uses_incremental_ledger_compute_positions(self):
        ...
        **确有一次 `compute_positions(...)` 调用**。
        ...
        src = (ROOT / "src" / "custos" / "core" / "trades" / "reconcile_positions.py").read_text(encoding="utf-8")
        tree = ast.parse(src)

        imported = {a.name for n in ast.walk(tree)
                    if isinstance(n, ast.ImportFrom) and n.module and n.module.endswith("incremental_ledger")
                    for a in n.names}
...
class TestWiredIntoDailyChain:
    ...
    SRC = (ROOT / "src" / "custos" / "pipeline" / "run_1700.py").read_text(encoding="utf-8")
