...
2026-08-06 review `src/custos/core/trades/` 时发现的缺口：
...
from custos.core.trades import reconcile_positions as rp  # noqa: E402
...
class TestReplay:
    def test_buy_then_partial_sell(self, tmp_path):
        led = _ledger(
            tmp_path,
            [
                _row("2026-08-03", "09:31:00", "600000", "买入", 1000, 10.0, 5.0),
    ...
    def test_out_of_order_rows_are_sorted(self, tmp_path):
        ...
        led = _ledger(
            tmp_path,
            [
                _row("2026-08-05", "10:00:00", "600000", "卖出", 500, 11.0),
...
class TestDiff:
    ...
    def test_qty_mismatch_flagged(self):
        d = rp.diff_positions(
            [{"代码": "600000", "持有数量": 600.0, "单位成本": 10.0}],
    ...
    def test_cost_float_residue_tolerated(self):
        ...
        d = rp.diff_positions(
            [{"代码": "600000", "持有数量": 100.0, "单位成本": 10.000000000001}],
    ...
    def test_real_cost_diff_flagged(self):
        d = rp.diff_positions(
            [{"代码": "600000", "持有数量": 100.0, "单位成本": 10.5}],
            [{"代码": "600000", "持有数量": 100.0, "单位成本": 10.0}],
        )
...
class TestReusesSingleSourceOfTruth:
    def test_replay_uses_incremental_ledger_compute_positions(self):
        ...
        **确有一次 `compute_positions(...)` 调用**。
        ...
        src = (
            ROOT / "src" / "custos" / "core" / "trades" / "reconcile_positions.py"
        ...
        imported = {
            ...
            and n.module
            and n.module.endswith("incremental_ledger")
            for a in n.names
        }
        ...
        fn = next(
            (
                n
                for n in ast.walk(tree)
                if isinstance(n, ast.FunctionDef) and n.name == "replay_ledger"
            ),
            None,
        )
...
class TestDefaultIsAdvisory:
    def test_strict_off_returns_zero_even_on_mismatch(
