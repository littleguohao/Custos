...
class TestRssSummaryFragments:
    ...
    def test_unparseable_stages_add_nothing(self):
        ...
        assert (
            run_0850._rss_summary_fragments(
                {
...
class TestLogStage:
    def test_tails_truncated_to_1000(self):
        ...
        entry = run_0850._log_stage(
            "stage1", r, "2026-07-17T08:50:00", "2026-07-17T08:50:10", 10.0
        )
...
class TestCollectionStatus:
    ...
    def _run(self, tmp_path, monkeypatch, failing: set[str], capsys):
        ...
        monkeypatch.setattr(
            ...
            lambda cmd, name: {
                ...
                "returncode": 0 if name not in failing else 1,
                "timeout": False,
                "stdout": "",
                "stderr": "",
                "out": "",
        ...
        log = json.loads(
            (tmp_path / "2026-07-20_0850_run_log.json").read_text(encoding="utf-8")
        )
    ...
    def test_all_ok_is_completed(self, tmp_path, monkeypatch, capsys):
        ...
        summary = [s for s in log["stages"] if s.get("stage") == "collection_summary"][
            0
        ]
        assert summary["failed_stages"] == [] and summary["ok"] is True
        assert "降级" not in printed

    def test_any_failure_is_degraded_and_lists_failed_stages(
        self, tmp_path, monkeypatch, capsys
    ):
        rc, log, printed = self._run(
            tmp_path, monkeypatch, {"overseas", "rss_collect"}, capsys
        )
