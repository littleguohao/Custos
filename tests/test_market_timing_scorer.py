...
class TestScoreAmv:
    ...
    def test_bear_scores_zero_not_neutral(self):
        ...
        """
        s, why = ms.score_amv(
            {"amv_0": {"effective_state": "空头", "amv_change_pct": -3.0}}
        )
    ...
    def test_long_scores_full(self):
        assert (
            ms.score_amv({"amv_0": {"effective_state": "做多", "amv_change_pct": 5.0}})[
                0
            ]
            == 15
        )
    ...
    def test_lock_overrides_daily_value(self):
        ...
        assert (
            ms.score_amv({"amv_0": {"effective_state": "空头", "amv_change_pct": 3.0}})[
                0
            ]
            == 0
        )
        ...
        s, _ = ms.score_macro(
            {
                "macro_policy": {
                    "monetary_policy": "宽松",
                    "fiscal_policy": "积极",
                    "credit_environment": "扩张",
                    "regulation_environment": "呵护市场",
                }
            }
        )
...
class TestScoreOverseas:
    @pytest.mark.parametrize(
        "avg,want", [(1.0, 10), (0.5, 7), (0.0, 5), (-0.5, 3), (-1.0, 1)]
    )
...
class TestStaleDetection:
    ...
    def test_missing_as_of_is_currently_fail_open(self):
        ...
        **不在补测试的这一轮改它** —— 改成 fail-closed 会降低评分、改变 live 择时行为，
        ...
        """
        assert ms.is_stale({}, "2026-08-07") is False, (
            "行为变了：请同步更新待办与本测试"
        )
...
class TestIsStaleHonorsProducerQuality:
    ...
    **11/15 分**，而归因文案是「涨跌比 1.24。」—— 完全看不出数据日未知。
    ...
    def test_score_degrades_to_neutral(self, quality):
        ...
        断言照样通过而评分毫无变化。
        """
        s, note = ms.score_breadth(
            {"date": self.DAY, "market_breadth": self._sec(quality, "")}
        )
    ...
    def test_vocabulary_covers_both_producers(self):
        ...
        from custos.pipeline.market_timing import market_timing_collector as mtc
...
class TestScoreIndices:
    ...
    def test_all_ma_above_scores_higher_than_all_below(self):
        up = ms.score_indices(
            self._one(
                above_ma25=True, above_ma60=True, above_ma144=True, above_ma240=True
            )
        )[0]
        dn = ms.score_indices(
            self._one(
                above_ma25=False, above_ma60=False, above_ma144=False, above_ma240=False
            )
        )[0]
