...
class TestRun1700MfeStageEcho:
    ...
    def _run(self, tmp_path, monkeypatch, mfe_out, mfe_ok):
        ...
        monkeypatch.setattr(
            pipeline_kit, "check_trading_day", lambda d: {"is_trading_day": True}
        )
    ...
    def test_full_coverage_reported_ok(self, tmp_path, monkeypatch, capsys):
        self._run(
            tmp_path,
            monkeypatch,
            "[OK] MFE/MAE 3/3 出数(complete) -> f.json",
            mfe_ok=True,
        )
        printed = capsys.readouterr().out
...
class TestSectorIndexSuccessRate:
    ...
    def test_low_success_rate_exits_nonzero_and_writes_status(
        ...
        fs = self._install(
            monkeypatch,
            PartialTQ(ok_codes={"880001.SH"}),
            [f"88{i:04d}" for i in range(1, 11)],
        )
        rc = fs.main(["--out", str(tmp_path), "--start", "20180101"])
        status = json.loads(
            (tmp_path / "_fetch_status.json").read_text(encoding="utf-8")
        )
        assert rc != 0
    ...
    def test_full_success_exits_zero_with_ok_status(
        self, tmp_path, monkeypatch, capsys
    ):
        from helpers_sector_tq import PartialTQ
    ...
    def test_threshold_is_configurable(self, tmp_path, monkeypatch):
        ...
        fs = self._install(
            monkeypatch, PartialTQ(ok_codes={"880001.SH"}), ("880001", "880002")
        )
...
class TestTqSectorSuccessRate:
    ...
    def test_main_exits_nonzero_on_low_success_rate(self, monkeypatch, tmp_path):
        ...
        assert rc != 0
        saved = json.loads(
            (tmp_path / "2026-07-17_tq_sector_map.json").read_text(encoding="utf-8")
        )
...
class TestPendingPositionMissingFields:
    ...
    def test_missing_cost_does_not_produce_zero_percentages(
        ...
        pos_path.write_text(
            json.dumps(
                [{"代码": "600000", "名称": "A", "持有数量": 100}], ensure_ascii=False
            ),
            encoding="utf-8",
        )
        monkeypatch.setattr(cm, "HOLDINGS_DIR", tmp_path / "data" / "holdings")
...
class TestSellFlyCoverage:
    ...
    def _base_with_closings(self, tmp_path, mfe_holdings):
        ...
        p.write_text(
            json.dumps(
                {"date": "2026-07-16", "holdings": mfe_holdings}, ensure_ascii=False
            ),
            encoding="utf-8",
        )
        return wr, base
    ...
    def test_partial_coverage_reported(self, tmp_path):
        wr, base = self._base_with_closings(
            tmp_path,
            [
                {
                    "code": "600000",
...
class TestWeeklyReviewIoReuse:
    ...
    def test_mfe_json_parsed_once_across_closings(self, tmp_path, monkeypatch):
        ...
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_text(
            json.dumps({"date": "2026-07-16", "holdings": []}), encoding="utf-8"
        )
    ...
    def test_mfe_index_globs_once_and_is_reusable(self, tmp_path):
        ...
        for day in ("2026-07-15", "2026-07-16"):
            (holdings / f"{day}_mfe_mae.json").write_text(
                json.dumps(
                    {"holdings": [{"code": "600000", "cost": 10.0, "mfe_pct": 5.0}]}
                ),
                encoding="utf-8",
            )
...
class TestNewsClassifyPrecedence:
    ...
    def test_policy_category_alone_is_policy(self):
        ...
        assert (
            pnd.classify({"category": "policy_official", "matched_themes": []})
            == "政策"
        )
        assert (
            pnd.classify({"category": "policy_consultation", "matched_themes": []})
            == "政策"
        )
    ...
    def test_official_category_needs_macro_theme(self):
        ...
        assert (
            pnd.classify({"category": "macro_official", "matched_themes": ["宏观政策"]})
            == "政策"
        )
        # 官方源但没命中宏观政策主题 → 不是政策(落到风向/信息/舆情)
        assert (
            pnd.classify({"category": "macro_official", "matched_themes": []}) != "政策"
        )
        assert (
            pnd.classify({"category": "company_official", "matched_themes": ["半导体"]})
            == "信息"
        )
    ...
    def test_market_keyword_wins_when_not_policy(self):
        ...
        assert (
            pnd.classify(
                {"category": "cn_financial_media", "matched_market_keywords": ["放量"]}
            )
            == "风向"
        )
...
class TestWeeklyReviewLocalDegradation:
    ...
    def test_malformed_closed_range_does_not_raise(self, tmp_path):
        ...
        got = wr.trading_days_of_week(
            tmp_path, ["2026-07-13", "2026-07-14", "2026-07-18"]
        )
    ...
    def test_whole_review_survives_dirty_amv(self, tmp_path):
        ...
        amv.write_text(
            json.dumps({"date": "2026-07-14", "amv_change_pct": "bad"}) + "\n",
            encoding="utf-8",
        )
            ...
            json.dumps(
                {
                    "channels": {
                        "feishu": {
                            "accounts": {
                                "default": {"appId": "cli_x", "appSecret": "s"}
                            }
                        }
                    }
                }
