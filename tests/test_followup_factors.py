...
def _write_ff(mdir, date, stocks, sectors=None):
    payload = {
        "date": date,
        "stock_rank": stocks,
        "sector_rank": {"concept": sectors or [], "industry": []},
    }
    (mdir / f"{date}_fund_flow_rank.json").write_text(
...
def test_fund_flow_single_day_unchanged(tmp_path):
    _write_ff(
        tmp_path,
        "2026-07-22",
        [{"code": "600000", "main_net_inflow": 3e8, "main_net_pct": 2.0}],
    )
    r = ec.load_fund_flow("2026-07-22", cumulative_days=1, market_dir=tmp_path)
    assert r["by_code"]["600000"]["main_net_inflow"] == 3e8 and r["files_used"] == [
        "2026-07-22"
    ]
...
def test_financial_factor_dixi_hit():
    ...
    assert set(r["hits"]) == {
        "perf_surge_ge_100",
        "net_profit_positive",
        "op_cashflow_positive",
        "real_earnings_cashflow",
        "roe_positive",
    }
...
def test_financial_factor_cashflow_missing_degrades():
    ...
    assert r["dixi_proxy"]["net_profit_positive"] is True
    assert (
        r["dixi_proxy"]["op_cashflow_positive"] is None
    )  # 现金流缺失→未确认(非 False)
    assert r["dixi_proxy"]["real_earnings_cashflow"] is False  # 不冒充成立
    assert (
        r["dixi_proxy"]["perf_surge_ge_100"] is True
        and r["dixi_proxy"]["roe_positive"] is True
    )
    assert (
        "net_profit_positive" in r["hits"] and "real_earnings_cashflow" not in r["hits"]
    )
...
def test_financial_factor_degrades():
    assert (
        fin.financial_factor("600000", None, _COLMAP, price=10.0)["available"] is False
    )  # 无财务数据(本机)
    assert (
        fin.financial_factor("600000", _FIN_DF, {}, price=10.0)["available"] is False
    )  # 无colmap
    assert (
        fin.financial_factor("600000", _FIN_DF, {"code": "c_code"}, price=10.0)[
            "available"
        ]
        is False
    )  # 必需列缺
    assert (
        fin.financial_factor("999999", _FIN_DF, _COLMAP)["available"] is False
    )  # 代码不在表
...
def test_financial_factor_index_lookup():
    df = pd.DataFrame(
        {"净利润": [5e8, -1e8], "经营活动产生的现金流量净额": [3e8, -2e8]},
        index=["600000", "000002"],
    )
    cm = {
        "code": "__index__",
    ...
    ohlcv = pd.DataFrame(
        {
            "date": [
                f"2026-0{m}-{d:02d}"
                for m, d in [(5, i) for i in range(1, 29)]
                + [(6, i) for i in range(1, 29)]
                + [(7, i) for i in range(20, 25)]
            ][:61],
        }
    )
        ...
        rows.append(
            {
                "date": dates[i],
                "open": op,
                "high": max(op, cl) * 1.005,
                "low": min(op, cl) * 0.995,
                "close": cl,
                "volume": 1_000_000 if i < 62 else 600_000,
                "amount": 1.0,
            }
        )
...
def test_simulate_trade_stop():
    ...
    assert (
        r_intra["reason"] == "stop" and r_intra["ret"] < 0 and r_intra["exit_idx"] == 6
    )
...
def test_simulate_trade_bbi_exit_win():
    ...
    bbi = pd.Series(
        [float("nan")] * 6 + [9.0, 9.0, 11.0, 11.0]
    )  # idx6,7 收>BBI；idx8,9 收<BBI
    r = bt.simulate_b1_trade(df, 5, bbi)
...
def test_evaluate_and_summarize_trades():
    ...
    assert trades and all(
        t["reason"] in ("stop", "bbi_exit", "open_end") for t in trades
    )
    s = bt.summarize_trades(
        [
            {"ret": 0.2, "reason": "bbi_exit", "holding": 8},
            {"ret": -0.05, "reason": "stop", "holding": 3},
        ]
    )
    assert (
        s["n"] == 2 and s["win_rate"] == 0.5 and s["payoff_ratio"] == 4.0
    )  # 0.2 / 0.05
...
def test_evaluate_trades_amv_long_only_gate():
    ...
    assert gated and all(
        t["entry_date"] >= dates[35] for t in gated
    )  # 只在做多区间进场
...
def test_simulate_time_stop_and_risk_frac():
    closes = [10.0] * 12
    lows = [9.9] * 12
    lows[5] = 9.5  # 进场 idx5 → 止损9.5,其后低点9.9不破
...
def test_summarize_expectancy_R():
    s = bt.summarize_trades(
        [
            {"ret": 0.10, "reason": "bbi_exit", "holding": 8, "r_multiple": 3.0},
...
def test_portfolio_equity_and_drawdown():
    p = bt.simulate_portfolio(
        _PF_TRADES, risk_pct=0.01, max_concurrent=5, max_pos_frac=0.2, max_gross=1.0
    )
...
def test_portfolio_concurrency_cap():
    p = bt.simulate_portfolio(
        _PF_TRADES, risk_pct=0.01, max_concurrent=1, max_pos_frac=0.2, max_gross=1.0
    )
...
def test_portfolio_topn_picks_highest_score():
    ...
    p1 = bt.simulate_portfolio_topn(
        [cA, cB], top_n=1, risk_pct=0.01, max_concurrent=5, max_pos_frac=0.2
    )
    assert p1["n_taken"] == 1 and abs(p1["final_equity"] - 1.02) < 1e-6  # 选了高分A
    assert (
        p1["selected_expectancy"] == 0.10 and p1["selected_win_rate"] == 1.0
    )  # 被选子集=A(+10%)
    p2 = bt.simulate_portfolio_topn(
        [cA, cB], top_n=2, risk_pct=0.01, max_concurrent=5, max_pos_frac=0.2
    )
...
def test_collect_all_yields_more_candidates():
    df = _mk([10.0 + 0.1 * i for i in range(45)])
    stub = lambda s, code: {"score": 100, "suggestion": "可买"}
    nonoverlap = bt.evaluate_trades(
        {"T": df}, scorer=stub, min_bars=30, collect_all=False
    )
...
def test_evaluate_trades_entry_gate():
    ...
    trades = bt.evaluate_trades(
        {"T": df}, scorer=stub, min_bars=30, collect_all=True, entry_gate=gate
    )
...
def test_alpha101_prefers_strong_close():
    ...
    base = _mk([10.0] * 6)
    base.loc[base.index[-1], ["open", "high", "low", "close"]] = [
        9.6,
        10.1,
        9.5,
        10.0,
    ...
    weak = _mk([10.0] * 6)
    weak.loc[weak.index[-1], ["open", "high", "low", "close"]] = [
        10.4,
        10.5,
        9.9,
        10.0,
    ...
    assert (
        s_strong > s_weak and bt.SCORERS["alpha101"](base, "T")["suggestion"] == "可买"
    )
...
def test_low_vol_and_momentum_selectors():
    ...
    assert (
        bt.SCORERS["low_vol"](calm, "T")["score"]
        > bt.SCORERS["low_vol"](choppy, "T")["score"]
    )
    # 动量：上涨序列得分为正、下跌为负
    up = _mk([10.0 + 0.1 * i for i in range(130)])
    down = _mk([30.0 - 0.1 * i for i in range(130)])
    assert (
        bt.SCORERS["momentum"](up, "T")["score"]
        > 0
        > bt.SCORERS["momentum"](down, "T")["score"]
    )
...
def test_macd_hist_and_j_macd_turn_gate():
    # MACD 柱在底部拐头应上行
    down_then_up = _mk(
        [20.0 - 0.5 * i for i in range(25)] + [7.5 + 0.3 * i for i in range(8)]
    )
...
def test_reversal_quality_selector():
    ...
    assert (
        q is not None and q["suggestion"] == "可买" and q["score"] >= 3
    )  # 多数条件命中
...
def test_portfolio_sizing_uses_risk_floor():
    ...
    p = bt.simulate_portfolio(
        t, risk_pct=0.01, max_concurrent=5, max_pos_frac=1.0, max_gross=1.0
    )
...
def test_amv_ledger_records_tail_merge(tmp_path):
    ...
    lines = [
        {
            ...
            "amv_change_pct": -1.35,
            "quality": "confirmed",
            "recorded_at": "2026-07-20T15:41:24+08:00",
        },
        {
            "date": "2026-07-24",
            "amv_change_pct": -4.23,
            "quality": "confirmed",
            "recorded_at": "2026-07-24T15:37:15+08:00",
        },
        # 同日冲突:先误报 -4.23,后更正 +1.67 → 后写入的覆盖
        {
            ...
            "amv_change_pct": -4.23,
            "quality": "confirmed",
            "recorded_at": "2026-07-27T10:09:13+08:00",
        },
        {
            "date": "2026-07-27",
            "amv_change_pct": 1.67,
            "quality": "confirmed",
            "recorded_at": "2026-07-27T15:37:34+08:00",
        },
        ...
        },
        {
            ...
            "amv_change_pct": 5.0,
            "quality": "confirmed",
            "recorded_at": "2026-07-29T15:00:00+08:00",
        },
        {
            "date": "2026-07-17",
            "amv_change_pct": -5.84,
            "quality": "confirmed",
            "recorded_at": "2026-07-17T20:31:52+08:00",
        },
    ]
    ...
    # 缺文件 → 空,不 raise
    assert (
        bt._amv_ledger_records("2015-01-01", None, ledger_path=tmp_path / "nope.jsonl")
        == []
    )
...
def test_kdj_j_scorer():
    ...
    assert (
        r is not None and r["suggestion"] == "可买" and r["score"] < 13
    )  # 单边下跌 J 深度超卖
...
def test_platform_pullback_gate_and_platform_stop():
    ...
    tr = bt.simulate_b1_trade(
        df, 96, bbi, stop_mode="pct", stop_pct=8.0, stop_override=10.2 * 0.98
    )
