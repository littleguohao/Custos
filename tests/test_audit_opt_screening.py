...
def _hits(*codes, date="2026-07-22"):
    return {
        ...
        "status": "ok",
        "formulas": [
            {"id": "F1", "hits": [{"code": c, "name": "测试股"} for c in codes]}
        ],
    }
...
def test_j_gate_rejects_nan_j(monkeypatch):
    ...
    monkeypatch.setattr(
        ec,
        "kdj",
        lambda df: {"available": True, "j": float("nan"), "j_prev": float("nan")},
    )
...
def test_j_gate_still_accepts_low_j_and_rejects_high(monkeypatch):
    monkeypatch.setattr(
        ec, "kdj", lambda df: {"available": True, "j": 5.0, "j_prev": 4.0}
    )
    assert len(_run_enrich(monkeypatch, {"600000": _bars()})["candidates"]) == 1
    monkeypatch.setattr(
        ec, "kdj", lambda df: {"available": True, "j": 50.0, "j_prev": 49.0}
    )
...
def test_perfect_b1_fit_nan_j_scores_zero():
    ...
    fit = ec.compute_perfect_b1_fit(
        df, daily_j=float("nan"), zx={"available": False}, pullback={"available": False}
    )
...
def test_score_candidate_unknown_wave_type_no_crash():
    ...
    cand = {
        ...
        "name": "测试股",
        "patterns": {},
        "wave": {"wave_type": "brand_new_wave", "available": True},
...
def test_fund_flow_sector_real_match_kept():
    ...
    assert (
        ec.fund_flow_of("600000", "光伏设备", wider)["sector_matched"]
        == "光伏设备及组件"
    )
...
def test_window_return_excludes_new_listing_when_asked():
    ...
    assert lp.window_return(dates, closes, "2026-01-01", "2026-06-30") == pytest.approx(
        0.8
    )
    assert (
        lp.window_return(
            dates, closes, "2026-01-01", "2026-06-30", exclude_new_listing=True
        )
        is None
    )
    # 老票（窗前已有 K 线）不受影响
    ...
    closes2 = [9.0] + closes
    assert lp.window_return(
        dates2, closes2, "2026-01-01", "2026-06-30", exclude_new_listing=True
    ) == pytest.approx(0.8)
...
def test_window_return_new_listing_flag_default_off():
    ...
    assert lp.window_return(
        ["2026-03-02", "2026-03-03"], [10.0, 12.0], "2026-01-01", "2026-06-30"
    ) == pytest.approx(0.2)
...
@pytest.mark.parametrize(
    "scorer_name", ["s_shape", "s_reversal", "momentum", "reversal_quality"]
)
...
def test_perfect_b1_fit_dks_uses_shared_series():
    ...
    df = _wavy(200)
    fit = ec.compute_perfect_b1_fit(
        df, daily_j=5.0, zx={"available": False}, pullback={"available": False}
    )
    series = dks_series(df["close"].astype(float).reset_index(drop=True))
    assert fit["components"]["dks_rising"]["dks"] == pytest.approx(
        float(series.iloc[-1])
    )
...
def test_firings_reusable_rejects_param_mismatch(tmp_path):
    f = tmp_path / "firings.json"
    f.write_text(
        _firings_payload(extra={"entry_filter": "reversal_k"}), encoding="utf-8"
    )
    assert bl.firings_reusable(f, _Args()) is False
...
def test_firings_reusable_does_not_parse_records_body(tmp_path, monkeypatch):
    ...
    real = json.loads
    monkeypatch.setattr(
        bl.json, "loads", lambda s, *a, **k: (sizes.append(len(s)), real(s, *a, **k))[1]
    )
    assert bl.firings_reusable(f, _Args()) is True
