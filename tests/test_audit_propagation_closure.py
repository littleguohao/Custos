...
class TestCompassQualityReachesLedger:
    ...
    def test_main_downgrades_and_skips_autofill_when_unverified(self, tmp_path, monkeypatch,
        ...
        market.mkdir()
        target = "2026-08-03"
        (market / f"{target}_market_timing_input.json").write_text("{}", encoding="utf-8")

        monkeypatch.setattr(sca, "LEDGER", led)
        monkeypatch.setattr(sca, "MARKET_DIR", market)
        monkeypatch.setattr(sca, "trading_day_status", lambda d: {"is_trading_day": True})
        monkeypatch.setattr(sca.compass_amv, "parse_amv_daily",
        ...
        assert json.loads(led.read_text(encoding="utf-8").strip())["quality"] == "unverified"
        mkt = json.loads((market / f"{target}_market_timing_input.json").read_text(encoding="utf-8"))
        ...
        monkeypatch.setattr(ec.concept_tags, "load_tags_meta",
                            lambda: ({"600000": ["银行"]},
                                     {"available": True, "stale": True,
                                      "date": "2026-07-27",
                                      "requested_date": "2026-08-03"}))
        monkeypatch.setattr(ec, "_load_json",
                            lambda p, d: {"themes": [{"theme_id": "T1", "theme_name": "银行",
                                                      "semantic_tags": ["银行"]}]})
