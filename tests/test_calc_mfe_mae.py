...
def _t(day, side, qty, price=10.0, code="600000", name="测试股"):
    return {"date": day, "time": "09:30", "code": code, "name": name,
...
class TestResolveOpenEntryDates:
    ...
    def test_oversell_treated_as_flat_not_crash(self):
        ...
        got = cm.resolve_open_entry_dates([_t("2026-06-10", "买入", 500),
                                           _t("2026-06-20", "卖出", 900)])
        assert "600000" not in got

    def test_avg_buy_date_is_quantity_weighted(self):
        got = cm.resolve_open_entry_dates([
            _t("2026-06-01", "买入", 3000),
            _t("2026-06-11", "买入", 1000)])
...
class TestNormalizeDateCol:
    ...
    def test_reset_index_unnamed_column_accepted(self):
        ...
        import pandas as pd
        df = pd.DataFrame({"high": [11.0]}, index=pd.to_datetime(["2026-06-10"])).reset_index()
...
class TestWindowAnchoring:
    ...
    def test_bars_after_target_excluded(self):
        ...
        import pandas as pd
        bars = pd.DataFrame({"date": ["2026-06-10", "2026-06-11", "2026-06-12"],
                             "high": [11.0, 12.0, 99.0], "low": [10.0, 10.5, 10.6]})
        d = bars.assign(_d=bars["date"].astype(str).str[:10])
...
class TestLedgerIntegration:
    ...
    def _ledger(self, tmp_path, rows):
        ...
        buf = io.StringIO()
        w = csv.DictWriter(buf, fieldnames=["成交日期", "成交时间", "代码", "名称", "交易类别",
                                            "成交数量", "成交价格", "成交金额", "费用"])
        w.writeheader()
        for r in rows:
            w.writerow(r)
    ...
    def _row(self, day, code, side, qty, price):
        return {"成交日期": day, "成交时间": "09:30:00", "代码": code, "名称": "测试股",
                "交易类别": side, "成交数量": qty, "成交价格": price,
                "成交金额": qty * price, "费用": 5.0}

    def test_real_ledger_columns_parsed(self, tmp_path):
        p = self._ledger(tmp_path, [self._row("2026-05-06", "600000", "买入", 1000, 10.0),
    ...
    def test_non_trade_category_ignored(self, tmp_path):
        ...
        p = self._ledger(tmp_path, [self._row("2026-06-01", "600000", "转债转入", 1000, 10.0),
                                    self._row("2026-06-10", "600000", "买入", 1000, 11.0)])
        got = cm.load_entry_dates(p)
        assert got["600000"]["entry_date"] == "2026-06-10"
