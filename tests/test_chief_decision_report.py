...
def _write(data, plans, day, *, risk=None, gate=None, holdings=None, b1=None,
           sectors=None, mt=MT_MD):
    if risk is not None:
...
def _run(data, plans, day, monkeypatch, **kw):
    ...
    cdr.main()
    return json.loads((data / "decisions" / f"{day}_chief_decision.json")
                      .read_text(encoding="utf-8"))


OK_GATE = {"market_quality": {"status": "pass", "quality_score": 0.9},
...
class TestPermissionGating:
    ...
    def test_happy_path_keeps_permission(self, env, monkeypatch):
        data, plans = env
        d = _run(data, plans, "2026-08-07", monkeypatch,
                 risk={"risk_level": "普通"}, gate=OK_GATE, holdings=[], b1=[], sectors=[])
    ...
    def test_unknown_or_blocked_quality_forbids(self, env, monkeypatch, status):
        ...
        d = _run(data, plans, "2026-08-07", monkeypatch,
                 risk={"risk_level": "普通"}, gate=gate, holdings=[], b1=[], sectors=[])
    ...
    def test_strong_risk_level_forbids(self, env, monkeypatch):
        data, plans = env
        d = _run(data, plans, "2026-08-07", monkeypatch,
                 risk={"risk_level": "强风控"}, gate=OK_GATE, holdings=[], b1=[], sectors=[])
        assert d["new_position_permission"] == "禁止"
    ...
    def test_no_increase_authorization_downgrades_permission(self, env, monkeypatch, val):
    ...
    def test_forbid_stays_forbid_not_downgraded_to_observe(self, env, monkeypatch):
        ...
        data, plans = env
        gate = {"market_quality": {"status": "blocked"},
...
class TestHoldingActions:
    HOLD = [{"code": "600000.SH", "name": "浦发", "action": "持有", "priority": "P3",
             "reason": ["结构完好"]}]
    B1 = [{"code": "600000", "final_action": "持有", "final_priority": "P3",
           "final_reason": "B1 结构完好"}]

    def test_b1_action_used_when_no_high_risk(self, env, monkeypatch):
        data, plans = env
        d = _run(data, plans, "2026-08-07", monkeypatch, risk={"risk_level": "普通"},
                 gate=OK_GATE, holdings=self.HOLD, b1=self.B1, sectors=[])
    ...
    @pytest.mark.parametrize("risk_action,want", [
        ("清仓", "清仓"), ("止损", "止损"), ("减仓", "减仓"), ("观察", "禁止加仓")])
    def test_high_risk_overrides_b1_by_severity(self, env, monkeypatch, risk_action, want):
        ...
        """
        data, plans = env
        risk = {"risk_level": "普通", "stock_risks": [
            {"code": "600000", "priority": "高", "action": risk_action, "reason": "破位"}]}
        d = _run(data, plans, "2026-08-07", monkeypatch, risk=risk, gate=OK_GATE,
                 holdings=self.HOLD, b1=self.B1, sectors=[])
        h = d["holding_actions"][0]
    ...
    def test_stale_technical_forces_wait(self, env, monkeypatch):
        ...
        d = _run(data, plans, "2026-08-07", monkeypatch, risk={"risk_level": "普通"},
                 gate=gate, holdings=self.HOLD, b1=self.B1, sectors=[])
        h = d["holding_actions"][0]
    ...
    def test_high_risk_beats_stale_technical(self, env, monkeypatch):
        ...
        d = _run(data, plans, "2026-08-07", monkeypatch, risk=risk, gate=gate,
                 holdings=self.HOLD, b1=self.B1, sectors=[])
...
class TestWatchlistAndForbidden:
    def test_watchlist_only_permitted_sectors_capped_at_3(self, env, monkeypatch):
        ...
        d = _run(data, plans, "2026-08-07", monkeypatch, risk={"risk_level": "普通"},
                 gate=OK_GATE, holdings=[], b1=[], sectors=sectors)
    ...
    def test_forbidden_always_has_the_three_hard_rules(self, env, monkeypatch):
        ...
        data, plans = env
        d = _run(data, plans, "2026-08-07", monkeypatch,
                 risk={"risk_level": "普通", "forbidden_actions": ["无计划追高", "自定义禁令"]},
                 gate=OK_GATE, holdings=[], b1=[], sectors=[])
        f = d["forbidden_actions"]
        for x in ("无计划追高", "因J值低直接补仓", "绕过risk_control开仓", "自定义禁令"):
            assert x in f
    ...
    def test_missing_market_timing_md_degrades(self, env, monkeypatch):
        ...
        data, plans = env
        d = _run(data, plans, "2026-08-07", monkeypatch, risk={"risk_level": "普通"},
                 gate=OK_GATE, holdings=[], b1=[], sectors=[], mt=None)
        assert d["market_state"] == "未知"
        assert d["new_position_permission"] in ("原则不允许", "仅观察，不得加仓", "禁止")

    def test_markdown_and_json_both_written(self, env, monkeypatch):
        data, plans = env
        _run(data, plans, "2026-08-07", monkeypatch, risk={"risk_level": "普通"},
             gate=OK_GATE, holdings=[], b1=[], sectors=[])
