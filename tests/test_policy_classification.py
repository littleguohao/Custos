...
from custos.datasource.news import rss_filter  # noqa: F401  确保模块可导入
from custos.core.paths import RSS_FILTER_CONFIG_FILE
...
def item(category="policy_official", themes=(), neg=(), mkt=()):
    return {
        "category": category,
        "matched_themes": list(themes),
        "matched_policy_negative": list(neg),
        "matched_market_keywords": list(mkt),
    }
...
def simulate(title, category):
    ...
    return {
        ...
        "matched_market_keywords": [
            w for w in CFG["market_keywords"] if w.lower() in t
        ],
    }
...
class TestNonPolicyOfficialNeedsMacroTheme:
    ...
    def test_macro_official_with_theme_is_policy(self):
        assert (
            pnd.classify(item(category="macro_official", themes=["宏观政策"])) == "政策"
        )

    def test_media_source_never_policy(self):
        assert (
            pnd.classify(item(category="cn_financial_media", themes=["宏观政策"]))
            != "政策"
        )
