...
def _df(
    end: str = "2026-07-22", periods: int = 80, close: float = 10.0
) -> pd.DataFrame:
...
class TestB5StFilterFailClosed:
    ...
    def test_enrich_keeps_named_candidates_when_st_filter_ok(self, monkeypatch):
        ...
        r = _enrich(
            monkeypatch, hits, {"600000": _df(), "600001": _df()}, index_df=_index_df()
        )
...
class TestB6ManualPoolFailureVisible:
    ...
    def test_status_degrades_when_pool_file_missing(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            manual_pools, "TDX_BLOCK_DIR", tmp_path
        )  # 空目录＝池文件缺失
        r = fs.screen_formulas(
            "2026-07-22",
...
class TestB7IndexFreshness:
    ...
    def test_stale_index_marks_degraded_and_drops_rs(self, monkeypatch):
        ...
        r = _enrich(
            monkeypatch,
            _hits(("600000", "浦发银行"), st_filter="ok"),
            {"600000": _df()},
            index_df=stale,
        )
    ...
    def test_index_loader_exception_is_recorded(self, monkeypatch):
        ...
        r = ec.enrich(
            "2026-07-22",
            hits_data=_hits(("600000", "浦发银行"), st_filter="ok"),
            ohlcv_loader=lambda c: _df(),
            index_loader=_boom,
...
class TestB8SectorScoreNaN:
    ...
    def test_score_all_flags_nan_sector_score(self):
        ...
        states = [
            {"theme_id": "T1", "sector": "船舶", "state": "主升", "score": float("nan")}
        ]
        r = sc.score_all(
            "2026-07-22",
            enriched=enriched,
