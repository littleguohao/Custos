# -*- coding: utf-8 -*-
"""Tests for run_2030 observability: run-log writing (shared pipeline_kit helpers)."""
...
class TestWriteRunLog:
    def test_structure(self, tmp_path, monkeypatch):
        monkeypatch.setattr(run_2030, "LOG_DIR", tmp_path)
        stage = run_2030._log_stage("calendar", {"ok": True, "returncode": 0, "timeout": False},
                                    "2026-07-19T20:30:00", "2026-07-19T20:30:01", 1.0)
        path = run_2030._write_run_log("2026-07-19", "closed", "2026-07-19T20:30:00",
                                       __import__("time").time(), [stage])
        log = json.loads(path.read_text(encoding="utf-8"))
        assert path.name == "2026-07-19_2030_run_log.json"
        assert log["date"] == "2026-07-19"
        assert log["status"] == "closed"
        assert log["script"] == "run_2030"
    ...
    def test_failed_status_with_stage_info(self, tmp_path, monkeypatch):
        monkeypatch.setattr(run_2030, "LOG_DIR", tmp_path)
        stage = run_2030._log_stage("daily_pipeline",
                                    {"ok": False, "returncode": 1, "timeout": False,
                                     "stdout": "boom", "stderr": "err"},
                                    "2026-07-19T20:30:00", "2026-07-19T20:30:30", 30.0)
        path = run_2030._write_run_log("2026-07-19", "failed", "2026-07-19T20:30:00",
                                       __import__("time").time(), [stage])
        log = json.loads(path.read_text(encoding="utf-8"))
    ...
    def test_best_effort_stage_keeps_completed_status(self, tmp_path, monkeypatch):
        monkeypatch.setattr(run_2030, "LOG_DIR", tmp_path)
        stages = [
            run_2030._log_stage("collect_fund_flow",
                                {"ok": False, "returncode": 1, "timeout": False, "stdout": "boom"},
                                "2026-07-19T20:30:00", "2026-07-19T20:30:05", 5.0,
                                note="best-effort，失败不中断"),
            run_2030._log_stage("daily_pipeline", {"ok": True, "returncode": 0, "timeout": False},
                                "2026-07-19T20:30:05", "2026-07-19T20:31:00", 55.0),
        ]
        path = run_2030._write_run_log("2026-07-19", "completed", "2026-07-19T20:30:00",
