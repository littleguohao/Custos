...
class TestCheck0850Status:
    ...
    def test_discovery_failure_refuses_reuse_even_if_completed(
        self, tmp_path, monkeypatch
    ):
        """核心回归:08:50 采集失败却写 completed 时,09:05 必须重采而非用空数据出报告。"""
        monkeypatch.setattr(run_0905, "LOG_DIR", tmp_path)
        self._write_0850_log(
            tmp_path,
            "2026-07-18",
            "completed",
            stages=[
                {"name": "overseas", "ok": False},
                {"name": "rss_collect", "ok": False},
    ...
    def test_degraded_with_healthy_discovery_may_reuse(self, tmp_path, monkeypatch):
        ...
        self._write_0850_log(
            ...
            "degraded",
            stages=[
                {"name": "overseas", "ok": True},
                {"name": "rss_collect", "ok": True},
                {"name": "rss_filter", "ok": True},
                {"name": "incremental", "ok": False},
            ],
        )
        reuse, note = run_0905._check_0850_status("2026-07-18")
        assert reuse is True
    ...
    def test_unreadable_log_falls_back(self, tmp_path, monkeypatch):
        monkeypatch.setattr(run_0905, "LOG_DIR", tmp_path)
        (tmp_path / "2026-07-18_0850_run_log.json").write_text(
