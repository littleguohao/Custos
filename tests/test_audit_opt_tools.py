...
class TestNetRetryStatusAware:
    ...
    def test_retry_after_header_honored(self, sleeps):
        ...
        assert sleeps == [7.0], "服务端明确给了 Retry-After 就该照办，而不是自作主张退避"
    ...
    def test_jitter_zero_is_exact_backoff(self, sleeps):
        ...
        net_retry.fetch_with_retry("http://x", session=s, tries=3, backoff=2.0, jitter=0.0)
...
class TestYahooParsing:
    def _payload(self, **meta):
        base = {"regularMarketPrice": 13000.0, "previousClose": 13000.0,
                "regularMarketTime": 1_760_000_000}
        base.update(meta)
        return {"chart": {"result": [{"meta": base}]}}

    def test_flat_change_pct_is_zero_not_none(self):
        out = _cim().parse_yahoo_payload("XIN9.FGI", self._payload(regularMarketChangePercent=0.0))
        assert out["change_pct"] == 0.0, "平盘 0.0 被 `if chg` 当成缺数据 → 下游误判缺采集"
        assert out["price"] == 13000.0

    def test_derived_change_pct_still_works(self):
        out = _cim().parse_yahoo_payload("X", self._payload(regularMarketPrice=110.0,
                                                            previousClose=100.0))
...
class TestOnlineQuotesRowTolerance:
    ...
    def test_tencent_partial_bad_row_keeps_good_rows(self, monkeypatch, capsys):
        rows = [["2026-07-16", "10.0", "10.1", "10.2", "9.9", "1000"],
    ...
    def test_sina_partial_bad_row_keeps_good_rows(self, monkeypatch, capsys):
        rows = [{"day": "2026-07-16", "open": "1", "high": "2", "low": "0.5",
                 "close": "1.5", "volume": "10"},
                {"day": "2026-07-17"},
                {"day": "2026-07-20", "open": "2", "high": "3", "low": "1.5",
                 "close": "2.5", "volume": "20"}]
        resp = type("R", (), {"json": lambda self: self._p})()
        resp._p = rows
        monkeypatch.setattr(oq, "fetch_with_retry", lambda *a, **k: resp)
...
class TestUrlCodeWhitelist:
    ...
    @pytest.mark.parametrize("code", ["", "60000", "6000001", "60000a", "600000&x=1",
                                      "../../etc/passwd", "0.1/y", None])
    ...
    def test_try_quote_swallows_validation_error(self, monkeypatch):
        ...
        assert chq._try_quote(chq._eastmoney_bj_quote, "bad!", "x", "2026-07-20") is None
...
def _full_section(name, as_of, quality="auto"):
    body = {"market_breadth": {"up_count": 3000, "down_count": 1000},
            "sentiment": {"limit_up_count": 50},
            "turnover": {"turnover_change_pct": 5.0},
            "amv_0": {"amv_change_pct": 1.0}}[name]
...
class TestInheritedUsesExpectedDay:
    def test_preclose_inherited_from_expected_day_is_fresh(self, guard_data):
        ...
        _write_prior(guard_data, PREV, ["amv_0", "market_breadth", "sentiment", "turnover"])
    ...
    def test_preclose_inherited_from_older_day_still_stale(self, guard_data):
        _write_prior(guard_data, "2026-07-10",
    ...
    def test_current_file_section_unaffected_by_expected_day(self, guard_data):
        ...
        market = {name: _full_section(name, PREV) for name in
                  ("amv_0", "market_breadth", "sentiment", "turnover")}
    ...
    def test_relaxation_never_adds_stale_or_blocked(self, guard_data, prior_day,
                                                   in_file_as_of, exp):
        ...
        **真子集** ⇒ 只可能把 stale 变回新鲜，绝不可能新增 stale；而 blocked 要求
        ...
        if r["status"] == "blocked":
            ...
            assert all(c["inherited"] or c["stale_as_of"] or
                       c["quality"] in {"missing", "raw_only"} for c in core)
...
class TestMergeAmvAsOf:
    def test_amv_0_gets_as_of_from_amv_0day(self, tmp_path, monkeypatch):
        ...
        mkt = json.loads((md / "2026-07-20_market_timing_input.json").read_text(encoding="utf-8"))
        assert mkt["amv_0"]["quality"] == "confirmed"
        assert mkt["amv_0"]["as_of"] == "2026-07-20", "权重 35 的块没有 as_of，无法做陈旧校验"
    ...
    def test_amv_0_as_of_from_observation_ledger(self, tmp_path, monkeypatch):
        ...
        (md / "0amv_observations.jsonl").write_text(json.dumps(
            {"date": "2026-07-20", "amv_change_pct": -3.0, "as_of": "2026-07-20",
             "quality": "confirmed"}) + "\n", encoding="utf-8")
        assert mim.main(["--date", "2026-07-20"]) == 0
        mkt = json.loads((md / "2026-07-20_market_timing_input.json").read_text(encoding="utf-8"))
...
class TestMergeFailureLeavesTrace:
    def test_merge_exception_writes_status_and_nonzero(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(mim, "merge_incremental",
    ...
    def test_unavailable_incremental_section_not_merged(self, tmp_path, monkeypatch):
        ...
        mim = _mim()
        inc = {"breadth": {"880005": {"name": "涨跌家数", "status": "unavailable",
                                      "reason": "样本不足"}}}
...
class TestBreadthRatioHonesty:
    def test_ratio_unavailable_without_real_total(self, monkeypatch):
        mtc = _mtc()
        monkeypatch.setattr(mtc, "_vipdoc_rows",
                            lambda code, count=5: [{"date": "2026-07-17", "close": 2000.0,
                                                    "high": None, "low": None, "amount": 1.0}])
        monkeypatch.setattr(mtc, "previous_confirmed_trading_day", lambda d: "2026-07-17")
        monkeypatch.setattr(mtc, "resolve_total_stocks", lambda: (None, "no truth source"))
...
class TestCalendarCache:
    ...
    def test_cache_invalidated_when_file_changes(self, tmp_path, monkeypatch):
        ...
        cfg.write_text(json.dumps({"overrides": {
            "2026-07-20": {"is_trading_day": True, "reason": "v1"}}}), encoding="utf-8")
        ...
        cfg.write_text(json.dumps({"overrides": {
            "2026-07-20": {"is_trading_day": False, "reason": "v2"}}}), encoding="utf-8")
        import os
...
class TestBatchHoldingTechnicalCache:
    def test_analysis_computed_in_process_and_cached(self, tmp_path, monkeypatch):
        ...
        rows = bht.build_summary([{"code": "600000", "name": "A"},
                                 {"code": "600000", "name": "A"},
                                 {"code": "000001", "name": "B"}], "2026-07-20")
