...
def test_aggregate_mainstream_vs_off():
    ...
    assert (
        r["in_mainstream"]["n"] == 3
        and abs(r["in_mainstream"]["expectancy"] - 0.0467) < 1e-3
    )
...
def test_aggregate_unclassified_excluded_from_off():
    ...
    r = sm.aggregate(
        [_trade("600000", 0.05), _trade("999999", -0.09)], {"600000": ["A"]}, top_k=1
    )
    assert (
        r["n_classified"] == 1
        and r["in_mainstream"]["n"] == 1
        and r["off_mainstream"]["n"] == 0
    )
...
def test_invert_members_norm_and_exclude():
    ...
    name_map = {
        "880201": {"tdx_type": "2", "name": "锂电"},
        "880300": {"tdx_type": "4", "name": "细分"},
        "地区X": {"tdx_type": "3", "name": "江西板块"},
    }  # 3=地区,应剔除
    c2s = sm.invert_members(
        members, exclude_types=True, name_map=name_map, norm=lambda cc: str(cc)[:6]
    )
