...
class TestSectorState:
    ...
    def test_support_needs_stage_and_score(self, env):
        ...
        r = {
            x["sector"]: x["trade_permission"]
            for x in g.build_sector_state("2026-08-07")
        }
...
class TestRiskDecision:
            ...
            [
                {
                    "code": "600000",
                    "action": "观察",
                    "b1_holding_state": {"final_priority": "P1"},
                }
            ],
        )
        x = g.build_risk_decision("2026-08-07")["stock_risks"][0]
    ...
    def test_sorted_high_first(self, env):
        _w(
            ...
            "holdings/2026-08-07_holding_review.json",
            [
                {"code": "600001", "action": "减仓"},
                {"code": "600000", "action": "清仓"},
            ],
        )
        r = g.build_risk_decision("2026-08-07")
    ...
    def test_default_reason_when_absent(self, env):
        ...
        assert (
            g.build_risk_decision("2026-08-07")["stock_risks"][0]["reason"]
...
class TestMainWritesBothArtifacts:
    def test_main(self, env, monkeypatch):
        ...
        g.main()
        risk = json.loads(
            (env / "risk" / "2026-08-07_risk_decision.json").read_text(encoding="utf-8")
        )
        sec = json.loads(
            (env / "sectors" / "2026-08-07_sector_state.json").read_text(
                encoding="utf-8"
            )
        )
        assert (
            risk["risk_level"] == "强风控"
            and risk["regime_directive"]["allow_add"] is False
        )
...
class TestNanScoreHandling:
    ...
    def test_main_survives_nan_score(self, env, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["x", "--date", "2026-08-07"])
        g.main()
        txt = (env / "sectors" / "2026-08-07_sector_state.json").read_text(
            encoding="utf-8"
        )
