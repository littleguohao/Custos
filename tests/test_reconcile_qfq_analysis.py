...
def _patch(monkeypatch, adj_fn):
    ...
    tdx = pd.DataFrame({"date": dates, "raw_close": raw,
                        "close": [x * 0.9 for x in raw]})     # 乘法：adj/raw≡0.9
...
class TestConventionDetection:
    ...
    def test_insufficient_overlap_reports_both_ranges(self, monkeypatch, capsys):
        ...
        """
        import pandas as pd
        monkeypatch.setattr(R, "_load_tdx", lambda code: (
            pd.DataFrame({"date": ["2021-09-01"], "raw_close": [10.0], "close": [9.0]}), "s"))
        monkeypatch.setattr(R, "_load_qlib", lambda code: (
            pd.DataFrame({"date": ["2010-01-04"], "close": [5.0]}), "s"))
    ...
    def test_missing_raw_close_refuses(self, monkeypatch, capsys):
        ...
        import pandas as pd
        monkeypatch.setattr(R, "_load_tdx", lambda code: (
            pd.DataFrame({"date": ["2021-09-01"], "close": [9.0]}), "no raw"))
        monkeypatch.setattr(R, "_load_qlib", lambda code: (
            pd.DataFrame({"date": ["2021-09-01"], "close": [9.0]}), "s"))
        assert R.detect_convention("600000") == 2
...
class TestLimitPct:
    ...
    @pytest.mark.parametrize("code,want", [
        ("600519", 10.0), ("000001", 10.0), ("300750", 20.0), ("688111", 20.0),
        ("689009", 20.0), ("920808", 30.0), ("830799", 30.0)])
...
class TestReport:
    ...
    def test_vacuous_ok_is_flagged(self, capsys):
        ...
        """
        rows = [{"code": "600000", "status": "ok", "bars": 200, "ratio_spread": 0.0,
                 "worst_ret_diff": 0.0, "n_mismatch": 0, "note": "adjust_events=0"},
                {"code": "600519", "status": "ok", "bars": 200, "ratio_spread": 1e-6,
                 "worst_ret_diff": 1e-6, "n_mismatch": 0, "note": "adjust_events=3"}]
    ...
    def test_mismatch_changes_exit_code(self, capsys):
        ...
        rows = [{"code": "600612", "status": "mismatch", "bars": 200,
                 "ratio_spread": 0.21, "worst_ret_diff": 0.11, "n_mismatch": 42,
                 "mismatch_days": ["2021-09-01", "2021-09-02"], "note": "加法调整"}]
        rc = R.report(rows)
    ...
    def test_thresholds_printed(self, capsys):
        ...
        R.report([{"code": "600000", "status": "ok", "bars": 100,
                   "ratio_spread": 0.0, "worst_ret_diff": 0.0, "n_mismatch": 0}])
        out = capsys.readouterr().out
        assert "阈值" in out

    def test_skip_and_error_counted_separately(self, capsys):
        """跳过/出错不能混进「一致」—— 那会把取不到数说成验证通过。"""
        rows = [{"code": "a", "status": "skip", "note": "无 qlib 数据"},
                {"code": "b", "status": "error", "note": "读取失败"},
                {"code": "c", "status": "ok", "bars": 100, "ratio_spread": 0.0,
                 "worst_ret_diff": 0.0, "n_mismatch": 0, "note": "adjust_events=2"}]
        R.report(rows)
        out = capsys.readouterr().out
