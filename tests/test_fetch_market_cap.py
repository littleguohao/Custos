...
def _row(code="600000", shares=1e10, close=10.0, name="测试股", free=None):
    ...
    return {
        ...
        "TRADE_DATE": "2024-06-28 00:00:00",
        "CLOSE_PRICE": close,
        "TOTAL_SHARES": shares,
        "FREE_SHARES_A": free
        if free is not None
        else (None if shares is None else shares * 0.8),
        "TOTAL_MARKET_CAP": mv,
...
class TestDiffEvents:
    ...
    def test_unchanged_shares_not_written(self):
        ...
        evs = mc.diff_events(
            {"600000": 1e10}, [_row(shares=1e10)], "2024-07-28", "2024-06-28"
        )
        assert evs == []
    ...
    def test_change_recorded_with_interval(self):
        ...
        evs = mc.diff_events(
            {"600000": 1e10}, [_row(shares=1.2e10)], "2024-07-28", "2024-06-28"
        )
        assert len(evs) == 1
        e = evs[0]
    ...
    def test_tiny_float_noise_not_treated_as_change(self):
        evs = mc.diff_events(
            {"600000": 1e10}, [_row(shares=1e10 + 1e-9)], "2024-07-28", "2024-06-28"
        )
        assert evs == []
    ...
    def test_invalid_rows_skipped(self):
        rows = [
            _row(code=""),
            _row(code="000001", shares=None),
            _row(code="300001", shares=0),
...
class TestSharesAsOf:
    def _events(self):
        return [
            ...
            {
                "code": "600000",
                "observed_on": "2020-06-28",
                "prev_sample": "2020-05-28",
                "total_shares": 1.5e10,
                "kind": "change",
            },
            {
                "code": "000001",
                "observed_on": "2019-03-28",
                "prev_sample": "2019-02-28",
                "total_shares": 2e10,
                "kind": "first_seen",
            },
        ]
...
class TestMarketCap:
    def _events(self):
        return [
            {
                "code": "600000",
                "observed_on": "2024-06-28",
                "prev_sample": "2024-05-28",
                "total_shares": 2.9352e10,
                "close": 8.23,
                "market_cap": 2.9352e10 * 8.23,
                "kind": "change",
            }
        ]
...
class TestLedgerIO:
    def test_merge_dedups_on_code_and_observed_on(self, tmp_path):
        ...
        assert (
            mc.merge_write(
                [{**e, "observed_on": "2024-07-28", "total_shares": 1.2e10}], p
            )["added"]
            == 1
        )
    ...
    def test_round_trip_and_atomic(self, tmp_path):
        p = tmp_path / "sc.jsonl"
        mc.merge_write(
            [{"code": "600000", "observed_on": "2024-06-28", "total_shares": 1e10}], p
        )
    ...
    def test_corrupt_lines_skipped(self, tmp_path):
        p = tmp_path / "sc.jsonl"
        p.write_text(
            '{"code":"600000","observed_on":"2024-06-28","total_shares":1}\n{bad\n\n',
            encoding="utf-8",
        )
...
class TestCli:
    def test_as_of_rejects_dates_before_mv_start(self, tmp_path, capsys):
        ...
        p.write_text(
            json.dumps(
                {"code": "600000", "observed_on": "2018-01-31", "total_shares": 1e10}
            )
            + "\n",
            encoding="utf-8",
        )
...
class TestSamplingRun:
    ...
    def _setup_ledger(self, tmp_path):
        ...
        samples.write_text(
            json.dumps({"sampled": ["2024-06-28"], "empty": []}), encoding="utf-8"
        )
    ...
    def test_out_of_order_backfill_rejected(self, tmp_path, capsys, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            mc, "fetch_trade_date", self._fake_fetch(calls, [_row(shares=1.2e10)])
        )
        rc = mc.main(
            [
                "--dates",
        ...
        assert json.loads(samples.read_text(encoding="utf-8"))["sampled"] == [
            "2024-06-28"
        ]
...
def test_backfill_only_before_mv_start_and_merges(tmp_path):
    ...
    res = mc.backfill_equity_history(
        ["600000"], before=mc.MV_START, out_path=out, progress=0, session=_FakeSession()
    )
