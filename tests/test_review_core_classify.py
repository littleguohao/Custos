...
class TestNoLivePriceIsHardStop:
    ...
    @pytest.mark.parametrize(
        "quote", [{}, {"price": None}, {"price": "n/a"}, {"price": float("nan")}]
    )
    ...
    def test_missing_price_wins_over_everything(self):
        ...
        p, a, _ = rc.classify(
            POS,
            _tech(trend_state="下跌", box20_position="破位"),
            [{"priority": "高", "reason": "止损"}],
            {"price": None},
            True,
...
class TestRiskAndLoss:
    ...
    def test_box_break_gives_p1(self):
        p, a, _ = rc.classify(
            POS, _tech(box20_position="下沿/破位区"), [], _q(), False, None
        )
    ...
    def test_mid_priority_risk_does_not_trigger_p1(self):
        ...
        p, _a, _ = rc.classify(
            POS, _tech(), [{"priority": "中", "reason": "x"}], _q(), False, None
        )
        assert p != "P1"
...
class TestB1Precedence:
    ...
    def test_b1_short_circuits(self, pri):
        p, a, r = rc.classify(
            ...
            _q(),
            False,
            {"final_priority": pri, "final_action": "B1动作", "final_reason": "B1理由"},
        )
    ...
    def test_b1_p3_does_not_short_circuit(self):
        ...
        p, a, _ = rc.classify(
            POS,
            _tech(box20_position="破位"),
            [],
            _q(),
            False,
            {"final_priority": "P3", "final_action": "B1动作", "final_reason": "x"},
        )
    ...
    def test_high_risk_reason_is_not_lost_when_b1_wins(self):
        ...
        p, a, r = rc.classify(
            POS,
            _tech(trend_state="下跌", box20_position="下沿/破位区"),
            [{"priority": "高", "reason": "已触发止损线"}],
            _q(price=9.0),
            False,
            {
                "final_priority": "P2",
                "final_action": "尾盘跌破BBI待收盘确认",
                "final_reason": "首日跌破",
            },
        )
    ...
    def test_no_annotation_when_no_high_risk(self):
        _p, _a, r = rc.classify(
            ...
            _q(),
            False,
            {"final_priority": "P2", "final_action": "a", "final_reason": "b1理由"},
        )
...
class TestBearRegime:
    ...
    def test_missing_change_pct_does_not_fabricate_rebound(self):
        _p, a, _ = rc.classify(
            POS, _tech(), [], {"price": 11.0, "date": DAY}, True, None
        )
        assert a != "反弹减仓评估"
...
class TestDefaultBuckets:
    def test_downtrend_is_observe_no_add(self):
        p, a, _ = rc.classify(
            POS,
            _tech(trend_state="下跌", box20_position="箱体上半区"),
            [],
            _q(),
            False,
            None,
...
class TestMaps:
    ...
    def test_quote_map_indexes_by_code(self, tmp_path, monkeypatch):
        ...
        (tmp_path / f"{DAY}_holding_quotes.json").write_text(
            '{"quotes": [{"code": "600000", "price": 10}], "as_of": "x"}',
            encoding="utf-8",
        )
