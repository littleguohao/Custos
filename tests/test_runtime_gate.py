...
def test_position_gate_blocked_exits_5_only_when_required():
    ...
    assert (
        rg.decide_exit_code(g, require_position_gate=True) == rg.EXIT_POSITION_BLOCKED
    )
...
def test_missing_sections_fail_closed():
    ...
    assert (
        rg.decide_exit_code({}, require_position_gate=True) == rg.EXIT_POSITION_BLOCKED
    )
...
def test_unknown_status_fails_closed():
    ...
    for bad in ("blockd", "unknown", "", None, "PASS"):
        assert (
            rg.decide_exit_code(
                {"market_quality": {"status": bad}}, require_quality=True
            )
            == rg.EXIT_QUALITY_BLOCKED
        )
        assert (
            rg.decide_exit_code(
                {"position_gate": {"status": bad}}, require_position_gate=True
            )
            == rg.EXIT_POSITION_BLOCKED
        )
...
def test_main_preclose_session_uses_prev_trading_day(monkeypatch):
    ...
    monkeypatch.setattr(
        "custos.core.runtime_guards.previous_confirmed_trading_day",
        lambda d: "2026-07-17",
    )
