...
class TestMovedScriptsRunAsMain:
    ...
    def test_no_syspath_bootstrap(self, rel):
        """包式化（2026-08-11, 阶段 4b）后入口脚本**不得再有 sys.path 引导**。

        custos 可编辑安装后 `from custos.xxx import ...` 在脚本与包两种模式下
        都能解析，sys.path 注入机制已删除；这条防止有人把它加回来
        （它会复活「同一模块两份对象」那类缺陷）。
        """
...
class TestToolsPathSingleSource:
    ...
    2026-08-11 包式化（阶段 4b）后 sys.path 引导已全删——脚本经 custos
    可编辑安装解析，`__file__` 引导不再是合法例外：

        TOOLS = BASE / "src" / "custos"           ❌ 已 import paths 之后重新推导
