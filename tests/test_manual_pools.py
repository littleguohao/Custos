...
def test_load_pool_hits_and_missing(tmp_path):
    d = _make_block_dir(tmp_path)
    pool = manual_pools.load_pool(
        "震荡", "2026-07-22", block_dir=d, name_map={"600150": "中国船舶"}
    )
...
def _registry_with_pool():
    return {
        "universe": {},
        "manual_pools": [
            {"id": "POOL_ZHENDANG", "block_name": "震荡", "enabled": True}
        ],
...
def test_screen_pool_plus_formula_merge(tmp_path, monkeypatch):
    ...
    r = fs.screen_formulas(
        ...
        stock_list=["600150"],
        name_map={"600150": "中国船舶"},
        call=lambda m, p, timeout: {
            "ok": True,
            "value": {"600150.SH": {"s": ["0", "1"]}},
            "error": None,
        },
    ...
    assert r["formulas"][1]["hits"] == [
        {"code": "600150", "name": "中国船舶", "signal_date": "2026-07-22"}
    ]
