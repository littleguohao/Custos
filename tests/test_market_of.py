...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("900901", "SH"),
        ("999999", "SH"),
        # 沪市指数白名单（与深市 000xxx 个股同形，不能按 "0" 前缀误判 SZ）。
        # 2026-08-25：000688 被误读成深市个股 sz000688（国城矿业），复盘里科创50
        # 错成 30.97/-8.64%。000001 有歧义（平安银行 vs 上证指数），保留 SZ。
        ("000688", "SH"),
        ("000300", "SH"),
        ("000905", "SH"),
        ("000852", "SH"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("430047", 2),
        ("600519", 1),
        ("880005", 1),  # 收敛前 get_market 把 880 落到默认 SZ
        ("000001", 0),
        ("300750", 0),
    ],
)
