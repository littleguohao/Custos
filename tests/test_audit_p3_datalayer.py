...
def _rows(n, start=0):
    return [
        {
            "SECURITY_CODE": f"{600000 + start + i:06d}",
            "TOTAL_SHARES": 1e10,
            "SECURITY_NAME_ABBR": "测试股",
            "CLOSE_PRICE": 10.0,
            "TOTAL_MARKET_CAP": 1e11,
            "SECURITY_TYPE": "A股",
            "NOTICE_DATE": "2024-04-29 00:00:00",
        }
        for i in range(n)
    ]
...
class TestPagedFetchIncomplete:
    ...
    @pytest.mark.parametrize(
        "mod,fn", [(pit, "fetch_period"), (mc, "fetch_trade_date")]
    )
    def test_empty_page_midway_raises(self, mod, fn):
        """声明 3 页，第 2 页空 → 限流，绝不能当成翻完了。"""
        s = _FakePagedSession(
            {1: _page(_rows(2), pages=3, count=6), 2: _page([], pages=3, count=6)}
        )
    ...
    def test_missing_result_section_raises(self, mod, fn):
        ...
        s = _FakePagedSession(
            {1: {"success": False, "code": 429, "message": "too many"}}
        )
        with pytest.raises(mod.FetchIncomplete):
    ...
    def test_max_pages_exhausted_raises(self, mod, fn):
        ...
        s = _FakePagedSession(
            {
                1: _page(_rows(2), pages=9, count=18),
    ...
    def test_complete_multipage_ok(self, mod, fn):
        s = _FakePagedSession(
            {
                1: _page(_rows(2), pages=2, count=4),
...
class TestMarketCapIncompleteNotPersisted:
    ...
    def test_incomplete_day_is_not_sampled_or_written(
...
def _amv_series(start: dt.date, closes: list) -> bytes:
    return b"".join(
        _amv_record(start + dt.timedelta(days=i), c) for i, c in enumerate(closes)
    )
...
def vdat_two_chains(tmp_path):
    ...
    path.write_bytes(
        ...
        + _GAP.join(
            [
                _amv_series(wrong_start, wrong_closes),
    ...
    for i, c in enumerate(true_closes):
        ...
        if i >= 20:
            lines.append(
                json.dumps(
                    {
                        "date": (true_start + dt.timedelta(days=i)).isoformat(),
                        "amv_change_pct": pct,
                        "quality": "confirmed",
                    }
                )
...
class TestLatestAmvQuality:
    ...
    def test_fallback_marked_unverified(self, vdat_two_chains):
        out = compass_amv.latest_amv(
            root=vdat_two_chains["root"], truth_path=vdat_two_chains["no_truth"]
        )
    ...
    def test_truth_matched_marked_verified(self, vdat_two_chains):
        out = compass_amv.latest_amv(
            root=vdat_two_chains["root"], truth_path=vdat_two_chains["truth"]
        )
    ...
    def test_parse_exposes_quality_too(self, vdat_two_chains):
        parsed = compass_amv.parse_amv_daily(
            since="1990-01-01",
...
class TestTdxRootValidation:
    ...
    def test_universe_enumeration_refuses_broken_default_root(
...
class TestC7DoesNotBreakOnlineFallback:
    ...
    def test_get_ohlcv_table_falls_back_online_when_root_broken(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.setattr(ltd, "TDX_ROOT", tmp_path / "no_such_tdx")
        monkeypatch.setattr(ltd, "_tdx_root_verified", set())
