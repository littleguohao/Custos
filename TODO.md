...
| # | 事项 | 性质 |
|---|---|---|
| 26 | **板块信息利用整体优化**（owner 2026-08-12 扩展：「当前板块信息的处理还是很弱，没有利用好 TDX 的板块信息，后续需要整体优化」——TQ 板块数据利用不足）**进展（2026-08-19，v0.77-v0.79）**：owner 定案主线口径——每日记录涨幅 top10 板块（涨停家数=资金流入代理+主力净流入）与跌幅 top10（跌停家数），1800 池（J<13）判命中「近期多次上榜=活跃」板块。`sector_daily_rank` 采集器（单日/回填/活跃名单）+ `sector_inflow_study` 离线对照研究已落地并跑完三轮：**假设证伪**（240 错位窗三 horizon 全翻转，明细 v0.79 + 已失效表）——采集器保留作回调四项原料，假设不接 1800。**+ 回调四项切换检查脚本化**（当前主攻子项）：主题切换/主题内分化/大小票/高低位四项检查做成确定性脚本并进**每日复盘报告**。原料大部分已有：`sector_phase`/`theme_tracker_report`（主题相位）、`fetch_market_cap`（市值层）、`holding_sector_mapper`（板块映射）、250 日高低点、sector_daily_rank 涨跌幅榜（v0.77 新增）；输出字段以 `04_pullback_rotation.md` §六 的字段表为准。**+ 主线题材判定重设计**（owner 2026-08-13：「目前不准需要重新调整」）：盘后 §4 与盘前 §5 的主线节已压缩并标注待重设计（v0.57）；「多次上榜=活跃」口径已证伪（v0.79），主线口径重设计仍悬空、随回调四项一并考虑 | **大 feature，回调四项切换检查进行中** |
