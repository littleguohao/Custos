...
> #59 剩余机会、#63 解析器下沉、#67 接口收敛、#72 规划层、#73/#74 注册表扩展）
...
| 75 | **`top_n>0` 路径个别 scorer 单格病态慢**：topn20_probe 中 `(1-2*TS_RANK(close,5))*TS_RANK(volume,10)` 与 Amihud 两格 >28 分钟未完（同配置 expr1/expr3 约 1 分钟/格），疑病态分支；另 r3 的 `j_low/base_low`（top_n=0）1800s 超时 ×2。均为 infra 性能问题，不影响判定语义 | R32 结论 7（2026-09-12 生产机实测） |
