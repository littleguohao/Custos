...
>
> 最后更新：2026-09-11（#69 修复落地（v0.207：--count 透传 + 强制显式），剩余生产机重跑。
> 当前活跃：#60 影子观察（待 owner 拍板）、#61 校准回测、#26 剩余子项挂起、
> #59 剩余机会、#63 解析器下沉、#67 接口收敛、#69 重跑取三轴终审读数）
...
| # | 单元 | 重跑什么 | 前置 |
|---|---|---|---|
| 69 | [R32](governance/research/R32_evolution_engine_smoke.md) | 生产机按 smoke_r2_joint 同口径重跑 `--grid-judge`，取回 VWAP 候选（双窗已过）的三轴终审读数（修复侧已入库 v0.207：`_grid_command` 透传 `--count` + `--joint`/`--grid-judge` 强制显式 `--count`） | pull 后重跑 ~72k tokens / ~25 分钟 |
