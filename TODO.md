...
>
> 最后更新：2026-09-23（R37 出场轴进化战役预注册落档——出场基因组×战役壳、
> 判据 C1~C5 + CTL-1~5 写死，待 owner 拍板 + Phase 1 工程（v0.263）；
...
| 79 | **R37 出场轴进化战役（出场基因组 × 战役壳 runner）**（owner 拍板方向：教义「杠杆在出场与仓位」，入场侧闭环后开出场轴）：**预注册已落档（2026-09-23，v0.263）**——入场钉死 V0+j_low 基底只动出场，判据 C1~C5（基准档 pct5_trail08、随机出场臂标尺自备）+ 战役控制 CTL-1~5（继续/转向/双向停止/预算帽 500+台账记账）跑数前写死。**剩余**：①owner 拍板（CTL 数值跑数前可修订一次）；②Phase 1 工程——战役壳 runner（内层 evolution_loop 批次+外层确定性控制器+台账 JSON 契约+钉测）与出场基因组编码/规则化变异算子；③生产机跑出场轴战役第一批（随机臂立标尺） | R37 预注册落档（2026-09-23）；Phase 0 出场轴审计在案（回测↔live 规则集不对称：breakeven/trailing/time_stop/cost_zone 回流从未发生；N 型破位等 live 信号未回测化=R38 立项候选） | 待 owner 拍板后开工 Phase 1 |
