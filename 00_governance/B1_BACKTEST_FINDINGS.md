...
## 5. 未决 / 后续
- **【最高优先·进行中】跨年份真样本外验证**：~~qlib 数据接入~~ **已完成**(2026-07-28,`--data-source qlib --start/--end`,
  含退市股+前复权,1999-11→2026-02;0AMV regime 起点已跟随 --start,1993 年起全历史可用)。**下一步**:用 qlib 宇宙做
  **跨年份 walk-forward**(早期校准/后期验证),**重点验 `reversal_quality_inv`(选丑反转)是否跨年份、牛熊都成立**。
  注意 2020-09→2021-07 数据缺口,分段设计窗口时避开或注明。
