...
- **买入价/止损价必须与盘面一致** ⇒ 用未复权（用户看盘看的是未复权价格）
- **技术指标（BBI/J/均线）需要价格连续** ⇒ 用前复权，否则除权日必然失真

业界标准做法是**分离**：指标用前复权算，展示价格用未复权。但这要改 `enrich_candidates`
等一批模块的数据入口，且要确认通达信客户端里 owner 实际看的是哪种复权模式
（若客户端看前复权图，那 live 用未复权算 J 值就与眼睛看到的不一致）。

**在此之前，M2 回测建议同时跑两个口径**（`--data-source tdx` 与 `qlib`）并对照，
不要只信一个。
