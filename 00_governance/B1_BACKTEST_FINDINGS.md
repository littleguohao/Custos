...
8. **横截面择优(top-N)不加值（已定论）**：同配置干净对照(2000样本/count500)——
   **`baseline`(随机top-5) 全面优于 `reversal_quality`**：收益 +43% vs +33%、回撤 13.6% vs 15.7%、
   收益/回撤 **3.16 vs 2.12**、子集期望 +2.64% vs +2.23%。即**无信息随机择优 ≥ 反转质量择优** →
   reversal_quality 甚至轻微负向。**结论：按价量/形态选股不加 alpha**(与 S_shape、b1_pullback 一致)。
   收益来自**择时(0AMV) + 分散 + 交易管理**，不是选股。**简单/随机择优即可，不需要花哨选择器。**
   (注:全池 69.6%/+6.41% 系 `collect_all` 重叠重复计数 + 幸存者双重高估，不可作基准；被选中子集才是真实每笔。)
9. **外部因子借鉴**：
