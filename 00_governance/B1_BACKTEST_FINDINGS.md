...
### `cost_zone_3` 被否决，但方向不算错

胜率 38.5%（全场最高）却期望R +0.557 < 基准 +0.624，均盈 9.76%（全场最低）——
典型的「砍掉慢热单」：其中一部分后来会大涨。材料的「低等马不涨就拍」是**人工判断
「该涨不涨」**，而机械的「3 日未脱离成本区」太粗糙。若要保留这条，判据应更贴近材料
（如结合是否站上 BBI、收盘价重心是否上升），而非单纯计时。
