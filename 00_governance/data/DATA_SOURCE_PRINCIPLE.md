...
**① 从未与任何独立序列对账。** grep 整个 `adjust_factors.py` 与其测试，**无对账代码**。
它决定所有价格，而且出过两个 bug（BJ 分支写反、`out[:500]` 截断保留最旧的权息事件——
方向正好反了，因为前复权只累乘"晚于 t 的除权日"、最新事件影响最大）。
**修了 bug 不等于验证了正确性。**

⇒ 现成的独立参照：`S_DATA` 的 qlib bundle **本身就是前复权**，可逐日比对（见
`QLIB_LOCAL_DATA.md`）。这是零成本的交叉验证，尚未做。
