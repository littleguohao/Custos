...
>
> 2026-07-16生产架构调整：正式报告采用“确定性脚本主链 + 轻量单Agent调用/投递”。生产工作流不再创建、调用、等待或校验独立专业Agent结果。
...
原有角色定义继续作为逻辑职责和研究模板保留，生产执行统一由脚本模块承担。

### 定时生产窗口

- `08:50`：采集持仓公告、海外行情和RSS候选，写入标准化文件；不生成、不投递正式报告。
- `09:05`：复用08:50采集结果，只执行质量门、B1/RiskDecision/ChiefDecision、渲染、校验和投递。
- `14:45`：采集目标日尾盘行情，运行严格持仓检查并投递尾盘报告。
- `15:15`：只提醒补充当日交易确认和0AMV，不生成正式复盘。
- `20:30`：核心事实齐全后运行确定性盘后流水线和最终复盘；不得现场修改代码。
