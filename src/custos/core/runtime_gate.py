...
def decide_exit_code(
    ...
    *,
    ...
    require_position_gate: bool = False,
) -> int:
    ...
    if (
        require_trading_day
        and (gate.get("calendar") or {}).get("is_trading_day") is not True
    ):
        return EXIT_NOT_TRADING_DAY
    if (
        require_quality
        and (gate.get("market_quality") or {}).get("status") not in _QUALITY_PASS
    ):
        return EXIT_QUALITY_BLOCKED
    if (
        require_position_gate
        and (gate.get("position_gate") or {}).get("status") not in _POSITION_PASS
    ):
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(
        description="运行门控:写 runtime_gate.json 并按开关返回退出码"
    )
    p.add_argument("--date", required=True)
    p.add_argument(
        "--data-session",
        choices=["preclose", "postclose"],
        default="postclose",
    ...
    p.add_argument(
        "--require-trading-day",
        action="store_true",
        help=f"非交易日 exit {EXIT_NOT_TRADING_DAY}",
    )
    p.add_argument(
        "--require-quality",
        action="store_true",
    ...
    expected = None
    if (
        a.data_session == "preclose"
    ):  # 盘前/盘中:当日 K 线尚不存在,期望数据日=最近确认交易日
    ...
    code = decide_exit_code(
        r,
        require_trading_day=a.require_trading_day,
