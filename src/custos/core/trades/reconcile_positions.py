...
## 成本差异的第三类（2026-08-19 #32 观察期定性）：口径差，非缺陷

`current_positions.json` 的 `单位成本` 来自券商快照，是**摊薄/保本成本**——
已实现盈亏摊进剩余持仓、按回合（清仓后重新起算）、含预估卖出佣金（~0.03%）；
回放是**移动平均成本**（卖出不改变剩余持仓单位成本）。同一持仓两口径可差数 %
（实测 600150：摊薄 35.514 vs 移动平均 34.499；920808：78.667 vs 81.000，
均与台账逐笔核算吻合）。另有快照侧 3 位小数舍入的 ~1e-5 尾差。
⇒ `cost_only_diff` 不构成本对账的告警信号；**硬信号只有数量**（`--strict`
也只看 `qty_mismatch_count`）。成本口径统一（若要）是独立课题，不在本模块。
