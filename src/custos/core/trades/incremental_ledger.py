...
def norm(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for f in FIELDS:
        if f not in df:
            df[f] = ""
    df = df[FIELDS]
    df["成交日期"] = pd.to_datetime(df["成交日期"], errors="coerce").dt.strftime(
        "%Y-%m-%d"
    )
    df["成交时间"] = (
        df["成交时间"].astype(str).str.strip().str.replace(r"\.0$", "", regex=True)
    )
...
def compute_positions(new: pd.DataFrame, current_rows: list[dict]) -> list[dict]:
    ...
    for _, t in new.iterrows():
        ...
        if cat == "买入" or cat in SHARE_CREDIT_CATEGORIES:
            ...
            pos["持有数量"] = new_qty
            pos["单位成本"] = (
                ((old_qty * old_cost) + (qty * price) + fee) / new_qty if new_qty else 0
            )
...
def _write_confirmation(day: str) -> None:
    confirmations = (
        json.loads(CONFIRM.read_text(encoding="utf-8")) if CONFIRM.exists() else {}
    )
    confirmations[day] = {
...
def _load_existing():
    ...
    if "_fingerprint" not in existing.columns:
        existing = existing.assign(
            _fingerprint=norm(existing).apply(fingerprint, axis=1)
            if len(existing)
            else []
        )
    return existing
...
def main(argv=None) -> dict:
    ...
    #    （`src/custos/core/trades/_no_trades_2026080{5,6,7}.json`，2026-08-10 清理）。
    ...
    incoming["_fingerprint"] = (
        incoming.apply(fingerprint, axis=1) if len(incoming) else []
    )
    ...
    if len(new):
        ...
        staged = [
            (
                _stage(
                    LEDGER,
                    lambda p: merged.to_csv(p, index=False, encoding="utf-8-sig"),
                ),
                LEDGER,
            ),
            (
                _stage(
                    STOCK_JSON,
                    lambda p: stock.to_json(
                        p, orient="records", force_ascii=False, indent=2
                    ),
                ),
                STOCK_JSON,
            ),
            (
                _stage(
                    ...
                    lambda p: p.write_text(
                        json.dumps(
                            positions, ensure_ascii=False, indent=2, default=str
                        ),
                        encoding="utf-8",
                    ),
                ),
