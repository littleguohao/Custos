...
# ⚠️ 碰撞（2026-08-25 排查）：白名单 8 码中 7 个同时存在深市个股
# （000010/000016/000688/000852/000903/000905/000906，sz*.day 均在 vipdoc 实测存在，
# 如 000688=国城矿业、000016=深康佳）——白名单让**裸码**永远=沪市指数，
# 个股必须显式带 .SZ 后缀（显式后缀优先级最高，不受影响）。全仓无裸码引用这些
# 个股（宇宙/持仓路径均带显式后缀），默认指指数是安全口径；新增裸码个股引用前
# 必须先改成显式后缀。
SH_INDEX_BARE = {
    "000010",
    "000016",
