...
def compute_sector_phase(
    close, lookback: int = PHASE_LOOKBACK, fractal: int = PHASE_FRACTAL
) -> dict[str, Any]:
    ...
    if len(hi) >= 2:
        a, b = hi[-2], hi[-1]
        top_div = bool(
            close_v[b] > close_v[a] and dif_v[b] < dif_v[a]
        )  # 价新高、DIF不创高=顶背离
    if len(hi) >= 3:
        p1, p2, p3 = hi[-3], hi[-2], hi[-1]
        three_peaks = bool(
            close_v[p1] < close_v[p2] < close_v[p3]
            and dif_v[p1] > dif_v[p2] > dif_v[p3]
        )  # 三打白骨精
...
def favorable_series(
    dates, close, lookback: int = PHASE_LOOKBACK, fractal: int = PHASE_FRACTAL
) -> dict[str, bool]:
    ...
    for t in range(n):
        if dif[t] <= 0:
            ...
            continue
        conf = [
            i for i in sh if i + fractal <= t and i >= t - lookback + 1
        ]  # 截至 t 已确认且在回看内(与 compute_sector_phase 同宽)
        exhausted = False
        if len(conf) >= 2:
            a, b = conf[-2], conf[-1]
        ...
        if not exhausted and len(conf) >= 3:
            ...
            exhausted = bool(
                c[p1] < c[p2] < c[p3] and dif[p1] > dif[p2] > dif[p3]
            )  # 三打
...
def load_sector_gate(
    ...
    for sec in members:
        ...
        try:
            df = pd.read_csv(p)
            fav = favorable_series(
                df["date"].tolist(), df["close"].tolist(), lookback=lookback
            )
    ...
    try:
        from custos.core.factors import sector_mainstream as _sm  # noqa: PLC0415
    ...
    code2sec: dict[str, list] = {
        c6: [s for s in secs if s in fav_by_sec] for c6, secs in _c2s.items()
    }
...
def build_phase_resolver(
    ...
    try:
        ...
        for sec, codes in members.items():
            for cc in codes:
                _c2s.setdefault(_norm6(cc), []).append(sec)
    code2sec: dict[str, list] = {
        c6: [s for s in secs if s in phase_by_sec] for c6, secs in _c2s.items()
    }
    code2sec = {c6: secs for c6, secs in code2sec.items() if secs}
    ...
    def resolve(code6: str) -> dict[str, Any]:
        ...
        return {
            ...
            "phase": rep.get("phase"),
            "any_exhausted": any(p.get("exhausted") for p in phases),
            "sectors": secs[:5],
            "n_sectors": len(phases),
        }
