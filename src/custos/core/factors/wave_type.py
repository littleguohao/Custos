...
WAVE_ACCEL_10D_GAIN = (
    25.0  # 待回测参数：高斜率加速（拉升段内最大10日涨幅%下限，i_low→i_high 段上计算）
)
...
def _find_rally_segment(
    df, lookback: int = WAVE_LOOKBACK
...
def detect_wave_type(df) -> dict[str, Any]:
    ...
    if n < WAVE_MIN_BARS:
        return {
            "wave_type": "unknown",
            "available": False,
            "detail": {"reason": f"K线不足{WAVE_MIN_BARS}根"},
        }
