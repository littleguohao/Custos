...
def rsi_divergence(
    ...
    try:
        ...
        prior_low_idx = (
            int(seg_l.iloc[:-min_gap].idxmin()) if len(seg_l) > min_gap else None
        )
        ...
        prior_r = (
            float(r.loc[prior_low_idx])
            if r.loc[prior_low_idx] == r.loc[prior_low_idx]
            else None
        )
        ...
        return {
            "available": True,
            "bullish": bool(price_new_low and rsi_higher),
            "price_new_low": price_new_low,
            "rsi_higher": rsi_higher,
            "cur_low": round(cur_low, 4),
