...
STRUCT_MA_PTS = (
    20.0  # 均线多头 + 低点抬高（复用 s_shape.compute_ma_structure 0-10 → ×2）
)
...
def detect_launch_segment(
    df: pd.DataFrame, lookback: int = LAUNCH_LOOKBACK
) -> dict[str, Any]:
...
def compute_long_structure(df: pd.DataFrame) -> dict[str, Any]:
    ...
    ma_pts = (
        (ma.get("points") or 0.0) / 10.0 * STRUCT_MA_PTS if ma.get("available") else 0.0
    )

    oh = compute_overhead_supply(df)
    oh_pts = (
        (oh.get("points") or 0.0) / 10.0 * STRUCT_OVERHEAD_PTS
        if oh.get("available")
        else 0.0
    )
    ...
    score = (
        (STRUCT_QSX_DKS_PTS if bull_stack else 0.0)
        + ma_pts
        + oh_pts
...
def detect_weekly_b1_resonance(
    ...
    try:
        ...
        return {
            "available": False,
            "hit": False,
            "reason": f"dep_missing:{type(exc).__name__}",
        }
    try:
        if daily_j is None:
            dj = kdj(df)
        ...
        return {
            "available": True,
            "hit": bool(daily_low and week_low),
            "daily_j": round(daily_j, 2),
            "weekly_j": round(week_j, 2),
            "daily_j_low": bool(daily_low),
            "weekly_j_low": bool(week_low),
...
def compute_b1_dual(df: pd.DataFrame, code: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "suggestion": "可买"
            if total >= 70
            else ("观望" if total >= 55 else "不买"),
            "struct_detail": struct,
            "reversal_detail": rev,
            "resonance_detail": res,
        }
...
def detect_breakout_pullback_b1(
    df: pd.DataFrame, code: str = "", j_threshold: float = 13.0, ph_tol: float = 0.98
) -> dict[str, Any]:
