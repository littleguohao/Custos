...
用法:
    uv run python src/custos/pipeline/feishu_report_publisher.py --report <md路径> \
...
from custos.core.paths import BASE, QUALITY_DIR  # noqa: E402
...
def load_credentials(env=None) -> dict:
    ...
    if not cfg_path.exists():
        raise FeishuError(
            f"缺少飞书凭据: 未设置 FEISHU_APP_ID/FEISHU_APP_SECRET 且找不到 {cfg_path}"
        )
    try:
...
def _post(
    url: str, *, headers=None, json_body=None, data=None, files=None, files_factory=None
) -> dict:
...
def check_delivery_gate(date: str, gate_path: Path | None = None) -> dict:
    ...
    if not path.exists():
        return {
            "ok": False,
            "reason": f"缺少运行门控文件 {path}(未跑 runtime_gate)",
            "gate": None,
        }
    try:
    ...
    if cal is not True:
        return {
            "ok": False,
            "reason": f"非交易日(calendar.is_trading_day={cal!r})",
            "gate": gate,
        }
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--gate-file", default="", help="指定门控 JSON 路径(默认按 --date 推导)"
    )
    ...
    if args.require_gate:
        chk = check_delivery_gate(
            args.date, Path(args.gate_file) if args.gate_file else None
        )
        if not chk["ok"]:
            print(f"[WARN] 门控阻断投递: {chk['reason']}", file=sys.stderr)
            print(
                json.dumps(
                    {"sent": False, "gate_blocked": True, "reason": chk["reason"]},
                    ensure_ascii=False,
                )
            )
    ...
    if args.dry_run:
        ...
        print(
            json.dumps(
                {"sent": False, "dry_run": True, "summary_len": len(summary)},
                ensure_ascii=False,
            )
        )
        return 0
    ...
    try:
        ...
        file_key = upload_report_file(
            token, report_path, f"{args.date}_{args.title}.md"
        )
        ...
        print(
            f"[WARN] feishu_report_publisher {'半成功' if partial else '失败'}: {exc} | 进度={progress}",
            file=sys.stderr,
        )
        print(
            json.dumps(
    ...
    print(
        json.dumps(
            {"sent": True, "file_key": file_key, "summary_len": len(summary)},
            ensure_ascii=False,
        )
    )
    return 0
