...
def fetch_names_for(codes, session=None, batch: int = EM_BATCH) -> dict[str, str]:
    ...
    """
    wanted = sorted(
        {str(c).split(".")[0].zfill(6) for c in codes if str(c).split(".")[0].strip()}
    )
    if not wanted:
        return {}
...
def fetch_from_tq(codes=None, progress_every: int = 500) -> dict[str, str]:
    ...
    try:
        codes = (
            ...
            else local_tdx_data.list_local_vipdoc_codes()
        )
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        for i, c in enumerate(codes, 1):
            ...
            if progress_every and i % progress_every == 0:
                ...
                print(
                    f"[INFO] TQ {i}/{len(codes)} 命中 {len(out)} 失败 {failed} "
                    f"({rate:.0f}只/s)",
                    file=sys.stderr,
                    flush=True,
                )
...
def fetch_from_tdx_protocol() -> dict[str, str]:
    ...
    try:
        ...
        print(
            f"[WARN] TDX 协议名称源失败: {type(exc).__name__}: {exc}", file=sys.stderr
        )
        return {}
...
def fetch_name_map(session=None) -> tuple[dict[str, str], str]:
    ...
    try:
        ...
        print(
            f"[WARN] 东财 clist 名称源失败: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
    return {}, "unavailable"
...
def _refuses_overwrite(
    ...
    if len(new_map) >= floor:
        return False
    diag.update(
        name_map_rejected_source=source,
    ...
    print(
        f"[WARN] 名称表拒绝落盘：{source} 只取到 {len(new_map)} 只，"
...
def resolve_name_map(
    ...
    if allow_fetch:
        ...
        if name_map:
            try:
                ...
                print(
                    f"[WARN] 名称缓存落盘失败（本次仍用在线表）: {exc}", file=sys.stderr
                )
            diag.update(
                name_map_source=source,
    ...
    if not names:
        diag.update(
            name_map_source="unavailable",
            name_map_size=0,
            st_filter="unavailable",
            name_map_reason=meta.get("reason"),
        )
        print(
            "[WARN] 名称表不可用（东财/TQ/mootdx/缓存均失败）：ST 硬排除失效",
            file=sys.stderr,
        )
        return {}, diag
    diag.update(
        name_map_source=meta.get("source") or "cache",
        name_map_size=len(names),
    ...
    if meta.get("stale"):
        print(
            f"[WARN] 名称表陈旧（generated_at={meta.get('generated_at')}, "
            f"age={meta.get('age_days')}天 > {NAME_MAP_MAX_AGE_DAYS}）："
            f"新被 ST 的股票可能不在表内，ST 硬排除不完全可信",
            file=sys.stderr,
        )
    return names, diag
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(
        description="构建股票名称缓存（TDX 协议优先，TQ/clist 兜底）"
    )
    ap.add_argument(
        "--source",
        choices=["auto", "clist", "tq", "mootdx"],
        default="auto",
        help="auto=TDX协议→TQ→clist；clist 受限流通常拉不全，全量构建走 auto（TDX 协议）",
    )
    ap.add_argument("--out", default=str(CACHE))
    ap.add_argument(
        "--limit", type=int, default=0, help="仅 tq 源：只处理前 N 只（排障用）"
    )
    ...
    if args.source == "auto":
        ...
        try:
            ...
            name_map, source = (
                fetch_all_from_clist(min_coverage=CLIST_MIN_COVERAGE),
                "eastmoney_clist",
            )
        except Exception as exc:  # noqa: BLE001
