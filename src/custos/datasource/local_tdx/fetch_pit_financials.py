...
用法:
  # 拉指定报告期(可多个)
  uv run python src/custos/datasource/local_tdx/fetch_pit_financials.py --periods 2024-03-31 2024-06-30
  # 批量补历史(2014 年起所有季度末)
  # ⚠️ 需留足一年跑道:2015 年信号的同比特征要用 2014 各期**当时可见**的版本,
  #    2015 年初的信号本身也要有已可见的 2014 财报,故起点必须比首个信号年早一年。
  uv run python src/custos/datasource/local_tdx/fetch_pit_financials.py --since 2014 --all-quarters
  # as-of 查询:某日可见的最新一期
