...
用法:
    uv run python src/custos/datasource/local_tdx/fetch_sector_index_history.py --out data/market/sector_index --start 20180101
    ...
    if existing_path.is_file():
        ...
        try:
            ...
            if {"date", "close"}.issubset(old.columns):
                new_frame = pd.concat(
                    [old[["date", "close"]], new_frame], ignore_index=True
                )
            else:
        ...
        if corrupt is not None:
            ...
            print(
                f"[WARN] {existing_path.name} 缓存损坏无法合并({corrupt}),"
                f"已隔离为 {quarantine.name};本次仅用新抓数据落盘,"
                f"历史深度截断,需不带 --incremental 全量重拉恢复",
                file=sys.stderr,
            )
            try:
...
def incremental_start(
    existing_path: Path, floor: str, overlap_days: int = OVERLAP_DAYS
) -> str:
...
def _describe(d) -> str:
    ...
    try:
        ...
        return (
            f"{type(d).__name__}(shape={getattr(d, 'shape', None)}, "
...
def resolve_period(
    tq, probe_code: str, start: str, wanted: str = ""
) -> tuple[str, str]:
...
def write_fetch_status(
    ...
    status = (
        "ok" if (total and rate >= min_rate) else ("empty" if not ok else "degraded")
    )
    payload = {
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--start", default="20180101", help="起始日 YYYYMMDD(TQ 会给到本地实有最早)"
    )
    ap.add_argument(
        "--period",
        default="",
        help=f"周期串(缺省自动探测: {', '.join(PERIOD_CANDIDATES)})",
    )
    ap.add_argument(
        "--incremental",
        action="store_true",
