...
def parse_yahoo_payload(symbol: str, data: dict) -> dict:
    ...
    if not isinstance(r, dict):
        raise ValueError(
            f"Yahoo chart result[0] 结构异常（symbol={symbol}）: {type(r).__name__}"
        )
    meta = r.get("meta") or {}
        ...
        try:
            ...
            if rows < 2:
                out[code] = {
                    "name": name,
                    "status": "unavailable",
                    "reason": f"样本不足（需 ≥2 根 K 线，实得 {rows}）",
                    "rows": rows,
                }
                continue
            last = df.iloc[-1]
            ...
            out[code] = {
                ...
                "previous_amount": (
                    float(prev["amount"])
                    if "amount" in df.columns and prev["amount"] == prev["amount"]
                    else None
                ),
                "date": str(last.name if hasattr(last.name, "strftime") else ""),
                "up_count": int(last.get("up_count", 0))
                if "up_count" in df.columns
                else None,
                "down_count": int(last.get("down_count", 0))
                if "down_count" in df.columns
                else None,
            }
        except Exception as e:
...
def _a50_sanity(result: dict) -> None:
    ...
    if isinstance(chg, (int, float)) and abs(chg) > 3:
        a50["suspect"] = True
        a50["note"] = (
            "change_pct 超过 ±3%，存在 previous_close 错位(换月/元数据滞后)风险，使用前需人工核对"
        )
...
def main(argv=None) -> int:
    ...
    result = {
        "date": target,
        "collected_at": cn_now().strftime("%Y-%m-%dT%H:%M:%S+08:00"),
    }
    ...
    try:
        ...
        try:
            ...
            result["a50_futures"] = {
                "error": str(e),
                "note": "A50 CFD unavailable via Yahoo, use web_search in report",
            }
    _a50_sanity(result)
    ...
    result["limit_ladder"] = {
        "note": "collected via tdx_screener in post-close enrichment"
    }
