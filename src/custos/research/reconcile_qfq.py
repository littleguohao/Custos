...
用法：
    ...
    uv run python src/custos/research/reconcile_qfq.py --auto 20

    # 指定代码
    uv run python src/custos/research/reconcile_qfq.py --codes 600000,000001,920808
"""
...
def pick_auto(n: int, cache_dir: Optional[pathlib.Path] = None) -> list[str]:
    ...
    ⚠️ `cache_dir` 显式传参而不是读模块全局：`src/` 下的模块既可能被
    ...
    **同一文件会成为两个模块对象**，模块级常量各存一份 —— monkeypatch 只影响其中一个，
    ...
    for p in sorted(pathlib.Path(cache_dir).glob("*.json")):
        ...
        score = sum(
            abs(float(e.get("songzhuangu") or 0)) * 10
            + abs(float(e.get("fenhong") or 0))
            for e in ev
        )
    ...
    if not scored:
        print(
            "[WARN] xdxr 缓存为空，无法自动挑票；请用 --codes 指定，"
            "或先跑一次带前复权的选股/回测把缓存焐热",
            file=sys.stderr,
        )
...
def detail(code: str, top: int = 25) -> int:
    ...
    **不分开看就会把数据源差异误判成自己的公式 bug，或反之。**
    ...
    print("=" * 104)
    print(
        f"分歧日 {len(bad)} 天，其中**落在事件日 ±2 根内**的 {n_ev_aligned} 天、"
    ...
    if len(over_q) and not len(over_t):
        ...
        for _, r in over_q.nlargest(min(5, len(over_q)), "ret_qlib").iterrows():
            print(
                f"        {r['date']}  qlib {r['ret_qlib']:+.4%}  vs  tdx {r['ret_tdx']:+.4%}"
            )
    ...
    if has_raw:
        ...
        for _, r in bad.nlargest(top, "ret_diff").iterrows():
            print(
                f"{r['date']:<12}{r['close_tdx']:>10.4f}{r['raw_close']:>11.4f}"
        ...
        for _, r in bad.nlargest(top, "ret_diff").iterrows():
            print(
                f"{r['date']:<12}{r['close_tdx']:>11.4f}{r['close_qlib']:>11.4f}"
    ...
    **③ 缺口对现有窗口是否构成实际约束。** `--cross-window` 用的 2022-01~2024-12
    ...
    try:
        ...
        if depths:
            ...
            n = len(depths)
            print(
                f"   抽样 {n} 只：根数 min {depths[0]} / 中位 {depths[n // 2]} / "
                f"max {depths[-1]}"
            )
            print(
                f"   最早日期：min {firsts[0]} / 中位 {firsts[n // 2]} / max {firsts[-1]}"
            )
...
def qlib_fields(code: str) -> int:
    ...
    **加法调整**（见 `detect_convention`）—— 要还原真实价格就需要调整量，
    ...
    for bd, inst in Q.code_to_qlib_dir(code, bundles):
        ...
        extra = [
            p.name
            for p in fdir.glob("*.bin")
            if p.stem.split(".")[0] not in ("open", "high", "low", "close", "volume")
        ]
        if extra:
            print(
                f"   ⇒ **s_data 未读的字段**: {extra}"
...
def detect_convention(code: str) -> int:
    ...
    if (
        t is None
        or q is None
        or "raw_close" not in (t.columns if t is not None else [])
    ):
    ...
    if _med(qlib_add) < 1e-4 and _med(qlib_mult) > 1e-3:
        ...
        print(
            "     ⇒ 要还原真实价格需要调整量 c：raw = qlib + c。仍在市的票可用 tdx 求 c，"
        )
        print(
...
def report(rows: list[dict]) -> int:
    ...
    for r in rows:
        ...
        print(
            f"{r['code']:<10}{r['status']:>8}{r.get('bars', '—'):>9}"
    ...
    if vacuous:
        print(
            f"⚠️ 其中 {len(vacuous)} 只是 **adjust_events=0**（窗口内无除权 ⇒ 因子恒为 1）"
...
def main() -> int:
    ...
    out = (
        pathlib.Path(a.out)
        if a.out
        else OUTDIR / f"{cn_now().strftime('%Y%m%d_%H%M')}_qfq_reconcile.json"
    )
