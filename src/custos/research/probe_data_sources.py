...
用法：
    uv run python src/custos/research/probe_data_sources.py                # 全部
    uv run python src/custos/research/probe_data_sources.py --only tq      # 只探 TQ
    uv run python src/custos/research/probe_data_sources.py --repeat 5     # 每项跑 5 次
"""
...
class Probe:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "ms_p50": round(statistics.median(self.samples) * 1000, 1)
            if self.samples
            else None,
...
def probe_mootdx(repeat: int) -> list[Probe]:
    ...
    out.append(
        Probe(
            "mootdx",
            "Reader.read_vipdoc_daily(SH)",
            "本地 .day 二进制，回测/live 的主数据源",
        ).run(lambda: L.read_vipdoc_daily(SAMPLE_SH), repeat)
    )
    out.append(
        Probe(
            "mootdx",
            "Reader.read_vipdoc_daily(BJ)",
            "BJ 是 xdxr 不覆盖的市场，用来暴露前复权降级",
        ).run(lambda: L.read_vipdoc_daily(SAMPLE_BJ), repeat)
    )
    out.append(
        Probe(
            "mootdx",
            "list_local_vipdoc_codes()",
            "回测 universe 来源。⚠️ 会随通达信下载变动（实测 5535→5536）",
        ).run(lambda: L.list_local_vipdoc_codes(), repeat)
    )
    out.append(
        Probe(
            "mootdx", "get_ohlcv_table(qfq)", "全链默认口径：vipdoc + 自算前复权"
        ).run(lambda: L.get_ohlcv_table(SAMPLE_SH, count=260), repeat)
    )
    out.append(
        Probe(
            "mootdx",
            "get_ohlcv_table(BJ, qfq)",
            "预期：qfq 失败 → 静默降级为未复权（attrs['adjust']=='none'）",
        ).run(lambda: L.get_ohlcv_table(SAMPLE_BJ, count=260), repeat)
    )
    out.append(
        Probe("mootdx", "Quotes.get_online_bars()", "在线 TDX 协议；量建连成本").run(
            lambda: L.get_online_bars(SAMPLE_SZ, offset=120), repeat
        )
    )
    out.append(
        Probe("mootdx", "Quotes.get_snapshot()", "实时快照").run(
            lambda: L.get_snapshot(SAMPLE_SH), repeat
        )
    )
    out.append(
        Probe("mootdx", "get_online_index()", "指数在线行情").run(
            lambda: L.get_online_index(SAMPLE_INDEX), repeat
        )
    )
    out.append(
        Probe(
            "mootdx", "get_stock_list()", "在线全代码表（与本地 vipdoc 枚举是两个口径）"
        ).run(lambda: L.get_stock_list(), repeat)
    )
    out.append(
        Probe(
            "mootdx", "Affair.get_financial_data()", "财务专项文件；只在研究链用"
        ).run(lambda: L.get_financial_data(), 1)
    )  # 下载量大，只跑一次
    return out
...
def probe_qfq(repeat: int) -> list[Probe]:
    ...
    out.append(
        Probe("qfq", "cache_age_days(SH)", "缓存新鲜度").run(
    ...
    out.append(
        Probe(
            "qfq",
            "BJ 复权标记是否可信",
            "预期暴露：0 事件仍盖章 qfq ⇒ 未复权被当成已复权",
        ).run(_bj_adjust_stamp, 1)
    )
    return out
...
def probe_tq(repeat: int) -> list[Probe]:
    ...
    out: list[Probe] = []
    try:
        from custos.datasource.local_tdx import tq_http as T
        from custos.datasource.local_tdx import tq_sector as S
    except Exception as exc:  # noqa: BLE001
    ...
    out.append(
        Probe("tq", "is_tdxw_running()", "进程级预检").run(
            lambda: S.is_tdxw_running(), repeat
        )
    )
    out.append(
        Probe("tq", "ping/get_match_stkinfo", "连通性探测").run(
            lambda: T.ping(), repeat
        )
    )
    out.append(
        Probe("tq", "get_market_snapshot", "持仓/盘中快照").run(
            lambda: T.snapshot(TQ_SH), repeat
        )
    )
    out.append(
        Probe("tq", "get_stock_info", "股票名称（ST 判定唯一依据）").run(
    ...
    out.append(
        Probe(
            "tq",
            "拦截 down_type=1",
            "验证代码级防护生效：应返回 unsafe_down_type 且不发请求",
        ).run(lambda: T.call("download_file", {"down_type": 1}), 1)
    )
    return out
...
def probe_eastmoney(repeat: int) -> list[Probe]:
    ...
    out: list[Probe] = []
    try:
        from custos.datasource.local_tdx import fetch_pit_financials as P
        ...
        out.append(
            Probe(
                "eastmoney",
                "PIT 财务(RPT_LICO_FN_CPD)",
                "以**公告日**为可见日；本地 TDX 无此能力。单期约 1.15 万行"
                "（≈5400 只 × 多种报表类型），page_size=500 ⇒ 约 24 页",
            ).run(lambda: P.fetch_period("2025-12-31"), 1)
        )
    except Exception as exc:  # noqa: BLE001
        ...
        out.append(p)
    try:
        from custos.datasource.local_tdx import fetch_market_cap as M

        out.append(
            Probe(
                "eastmoney",
                "真市值(RPT_VALUEANALYSIS_DET)",
                "2018-01-02 起；替掉成交额代理",
            ).run(lambda: M.fetch_trade_date("2025-12-31"), 1)
        )
    except Exception as exc:  # noqa: BLE001
...
def main() -> int:
    ...
    ap.add_argument(
        "--repeat", type=int, default=3, help="每项重复次数（默认 3，取中位）"
    )
    ap.add_argument(
        "--out", default="", help="报告 JSON 路径（默认 artifacts/logs/data_probe/）"
    )
    a = ap.parse_args()
