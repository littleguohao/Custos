...
def cmd_backtest_diff(
    sample: int, seed: int, s_data_root: str, extra: Optional[list[str]] = None
) -> int:
    ...
    for k, label, pct in [
        ...
        f = (
            (lambda v: f"{v * 100:+.2f}%" if isinstance(v, (int, float)) else "—")
            if pct
    ...
    for name, s in (("未复权", u), ("前复权", a)):
        ...
        parts = [
            f"{k} {v['n']}({v['n'] / tot:.0%})"
            for k, v in sorted(rs.items(), key=lambda kv: -kv[1].get("n", 0))[:4]
        ]
...
def main() -> int:
    ...
    ap.add_argument(
        "--compare", action="store_true", help="与前复权源逐日对比（精确识别）"
    )
