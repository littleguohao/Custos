...
用法：
    uv run python src/research/m2_stop_sweep.py --sample 300      # 小样本试跑
    uv run python src/research/m2_stop_sweep.py                   # 全部
    uv run python src/research/m2_stop_sweep.py -j 6              # 6 进程并行
    uv run python src/research/m2_stop_sweep.py --only B_stop_pct # 只跑一组
    uv run python src/research/m2_stop_sweep.py --report-only     # 只重出报表
    uv run python src/research/m2_stop_sweep.py --cross-window    # 2022-2024 复核
...
if str(BASE / "src" / "core") not in sys.path:
    sys.path.insert(0, str(BASE / "src" / "core"))

from paths import LOGS  # noqa: E402

SCRIPT = BASE / "src" / "research" / "backtest_factors.py"
