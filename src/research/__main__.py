...
用法（沿用项目既有约定「按路径调脚本」，不需要设 PYTHONPATH）::
    ...
    uv run python src/research/__main__.py                    # 列出全部工具与状态
    uv run python src/research/__main__.py backtest_factors --help
    uv run python src/research/__main__.py m2_stop_sweep --sample 300
...
def _listing() -> int:
    print("研究 / 回测工具\n")
    print("  uv run python src/research/__main__.py <名字> [参数...]\n")
    for kind in ORDER:
