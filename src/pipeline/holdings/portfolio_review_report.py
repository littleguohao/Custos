...
def main():
    ...
    market_state=extract(r'状态：\*\*(.*?)\*\*',mt,'未知'); position=extract(r'建议总仓位：\*\*(.*?)\*\*',mt,'待确认')
    reviews=[]
    for r in tech:
        hold=b1.get(str(r.get('code')),{}); priority=hold.get('final_priority'); action=hold.get('final_action'); reasons=[]
        if not priority or not action:
            priority,action,reasons=classify(r)
        else:
            reasons=[hold.get('final_reason')]+[x.get('reason') for x in hold.get('signals',[])[1:3]]
        reviews.append({'code':str(r.get('code')),'name':r.get('name',''),'position_pct':r.get('position_pct'),'pnl_pct':r.get('holding_pnl_pct'),
          ...
          'action':action,'priority':priority,'reason':[x for x in reasons if x], 'b1_holding_state':hold})
    ...
    lines=['# portfolio_review 每日持仓研判','',f'日期：{a.date}','', '## 1. 总体持仓风险','',f'- market_timing：**{market_state}**',f'- 建议总仓位：**{position}**','- 原则：低位指标不能覆盖趋势、板块与风险规则。','', '## 2. 持仓逐只研判','', '| 优先级 | 代码 | 名称 | 仓位 | 盈亏 | 趋势/位置 | 动作 | 理由 |','|---|---|---|---:|---:|---|---|---|']
