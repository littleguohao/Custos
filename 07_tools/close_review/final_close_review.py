...
def main():
    ...
    for c,p in pmap.items():
        ...
        revalued.append({'code':c,'name':p.get('名称'),'quantity':qty,'cost':cost,'close':close,'latest_date':t.get('latest_date'),'market_value':mv,'pnl':pnl,'pnl_pct':pnl_pct,'position_pct':mv/total_assets if total_assets else 0,'trend':t.get('trend_state'),'box':t.get('box20_position'),'sector':sec,'index':index_name(c)})
    ...
    lines += ['',f'## 3. 持仓复盘（{valuation_label}）','', '| 代码 | 名称 | 数据日 | 参考价 | 成本 | 参考盈亏 | 参考仓位 | 个股走势 | 板块走势 |','|---|---|---|---:|---:|---:|---:|---|---|']
    for x in revalued:
        s=x['sector']; lines.append(f"| {x['code']} | {x['name']} | {x['latest_date']} | {x['close']:.2f} | {x['cost']:.3f} | {x['pnl_pct']:+.2%} | {x['position_pct']:.1%} | {x['trend']}/{x['box']} | {s.get('trend_state','未知')}/{s.get('stage','未知')} |")
