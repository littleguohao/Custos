...
def main():
    ...
    lines += ['',f'## 3. 持仓复盘（{valuation_label}）','', '| 代码 | 名称 | 价格日/时间 | 收盘价 | 成本 | 收盘盈亏 | 重估仓位 | 技术日 | 个股走势 | BBI持仓依据 | 板块走势 |','|---|---|---|---:|---:|---:|---:|---|---|---|---|']
    for x in revalued:
        s=x['sector']; bbi=x['bbi']; close_text='缺失' if x['close'] is None else f"{x['close']:.2f}"; pnl_text='缺失' if x['pnl_pct'] is None else f"{x['pnl_pct']:+.2%}"; pos_text='缺失' if x['position_pct'] is None else f"{x['position_pct']:.1%}"; lines.append(f"| {x['code']} | {x['name']} | {x['price_date'] or '缺失'} {x['price_time'] or ''} | {close_text} | {x['cost']:.3f} | {pnl_text} | {pos_text} | {x['technical_date']} | {x['trend']}/{x['box']} | {bbi['state']}；{bbi['reminder']} | {s.get('trend_state','未知')}/{s.get('stage','未知')} |")
