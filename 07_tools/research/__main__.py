...
用法（沿用项目既有约定「按路径调脚本」，不需要设 PYTHONPATH）::
    ...
    uv run python 07_tools/research/__main__.py                    # 列出全部工具与状态
    uv run python 07_tools/research/__main__.py backtest_factors --help
    uv run python 07_tools/research/__main__.py m2_stop_sweep --sample 300

也支持包形式（需要 `07_tools` 在 sys.path 上）::

    PYTHONPATH=07_tools uv run python -m research
...
def _listing() -> int:
    print("研究 / 回测工具\n")
    print("  uv run python 07_tools/research/__main__.py <名字> [参数...]\n")
    for kind in ORDER:
